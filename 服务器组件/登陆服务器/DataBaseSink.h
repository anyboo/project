...
private:
	...
	//锁定机器
    bool OnRequestLockJiQi(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize);
	//绑定帐号
	bool OnRequestSafeBind(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize);
	//解除绑定
	bool OnRequestSafeUnbind(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize);
	//操作保险柜 2012-4-10
	bool OnRequestOperMoney(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize);

	//用户存钱 2012-4-12
	bool OnRequestGiveMoney(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize);

	//保险柜密码 2012-4-12
	bool OnRequestBankPass(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize);


	//赠送记录 2012-4-12
	bool OnRequestGiveRecord(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize);

	bool OnRequestUserPass(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize);

	bool OnRequestUserWrite(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize);
...
private:
	...
	//注册存储过程
	LONG SPRegisterAccounts(LPCTSTR pszAccounts, LPCTSTR pszPassword, LPCTSTR pszSpreader, WORD wFaceID, BYTE cbGender, DWORD dwClientIP, LPCTSTR pszComputerID, LPCTSTR pszBankpass, LPCTSTR pszIdno, LPCTSTR pszEmail, LPCTSTR pszMobile);
	//上传存储过程
