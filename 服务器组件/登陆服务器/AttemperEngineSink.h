...
private:
    ...
    //绑定机器
	bool OnDBSafeBind(DWORD dwContextID, VOID * pData, WORD wDataSize);
	//解除绑定
	bool OnDBSafeUnbind(DWORD dwContextID, VOID * pData, WORD wDataSize);
	//操作保险柜 2012-4-10
	bool OnDBOperMoney(DWORD dwContextID, VOID * pData, WORD wDataSize);
	//刷新保险柜 2012-4-10
	bool OnDBRefreshMoney(DWORD dwContextID, VOID * pData, WORD wDataSize);

	//赠送 2012-4-12
	bool OnDBGiveMoney(DWORD dwContextID, VOID * pData, WORD wDataSize);

		//赠送 2012-4-20
	bool OnDBBankPass(DWORD dwContextID, VOID * pData, WORD wDataSize);

		//赠送 2012-4-20
	bool OnDBGiveRecord(DWORD dwContextID, VOID * pData, WORD wDataSize);
	//网络函数
private:
	//登录消息处理
