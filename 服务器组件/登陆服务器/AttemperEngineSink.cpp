...
bool __cdecl CAttemperEngineSink::OnEventDataBase(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	case DBR_GP_SAFE_BIND:			//绑定机器
		{
			return OnDBSafeBind(dwContextID, pData, wDataSize);
		}
	case DBR_GP_SAFE_UNBIND:		//解除绑定
		{
			return OnDBSafeUnbind(dwContextID, pData, wDataSize);
		}
	case DBR_GP_OPERMONEY_RESULT:  //操作保险柜结果 2012-4-10
		{
			return OnDBOperMoney(dwContextID, pData, wDataSize);
		}
	case DBR_GP_REFRESHEMONEY_RESULT:  //刷新保险柜结果  2012-4-10
		{
			return OnDBRefreshMoney(dwContextID, pData, wDataSize);
		}
	case DBR_GP_GIVEMONEY_RESULT:  //赠送操作结果 2012-4-12
		{
			return OnDBGiveMoney(dwContextID, pData, wDataSize);
		}
	case DBR_GP_BANKPASS_RESULT:
		{
			return OnDBBankPass(dwContextID, pData, wDataSize);
		}
	case DBR_GP_GIVERECORD_RESULT:
		{
			return OnDBGiveRecord(dwContextID, pData, wDataSize);
		}
	}
...
//绑定机器
bool CAttemperEngineSink::OnDBSafeBind(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//合法判断
	...
	DBR_GP_BindResult *pBindResult=(DBR_GP_BindResult *)pData;

	CMD_GP_BindResult BindResult={0};
	BindResult.cbSuccess=pBindResult->cbSuccess;

	//发送消息
	m_pITCPNetworkEngine->SendData(dwContextID,MDM_GP_USER,SUB_GP_SAFE_BIND,&BindResult,sizeof(BindResult));
...
//解除绑定
bool CAttemperEngineSink::OnDBSafeUnbind(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//合法判断
	ASSERT(LOWORD(dwContextID)<m_pInitParamter->m_wMaxConnect);
	if ((m_pBindParameter+LOWORD(dwContextID))->dwSocketID!=dwContextID) return true;
	...
	//发送消息
	m_pITCPNetworkEngine->SendData(dwContextID,MDM_GP_USER,SUB_GP_SAFE_UNBIND,&BindResult,sizeof(BindResult));
...
bool CAttemperEngineSink::OnDBLogonSuccess(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	SendPacket.AddPacket(pDBRLogonSuccess->szBankPassword,CountStringBuffer(pDBRLogonSuccess->szBankPassword),DTP_BANK_PASS);
	pCMDLogonSuccess->JiQi=pDBRLogonSuccess->JiQi;
	...
	case SUB_GP_REGISTER_ACCOUNTS:		//注册帐号
		{
			...
			RegisterAccounts.cbGender=(pRegisterAccounts->cbGender==1)?1:0;
			...
			lstrcpyn(RegisterAccounts.szBankpass,pRegisterAccounts->szBankpass,sizeof(RegisterAccounts.szBankpass));
			lstrcpyn(RegisterAccounts.szIdno,pRegisterAccounts->szIdno,sizeof(RegisterAccounts.szIdno));
			lstrcpyn(RegisterAccounts.szEmail,pRegisterAccounts->szEmail,sizeof(RegisterAccounts.szEmail));
			lstrcpyn(RegisterAccounts.szMobile,pRegisterAccounts->szMobile,sizeof(RegisterAccounts.szMobile));
			//变量定义
			VOID * pData=NULL;
			tagDataDescribe DataDescribe;
...
bool CAttemperEngineSink::OnSocketMainUser(WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	switch (wSubCmdID)
	{
		case SUB_GP_SAFE_BIND:			//绑定操作
		{
			//效验参数
			ASSERT(wDataSize==sizeof(CMD_GP_SafeBind));
			if (wDataSize!=sizeof(CMD_GP_SafeBind)) return false;

			//解析变量
			CMD_GP_SafeBind *pSafeBind = (CMD_GP_SafeBind*)pData;

			//设置变量
			DBR_GP_SafeBind SafeBind;
			ZeroMemory(&SafeBind, sizeof(SafeBind));
			SafeBind.dwUserID = pSafeBind->dwUserID ;
			lstrcpyn(SafeBind.szPassWord,pSafeBind->szPassWord,CountArray(SafeBind.szPassWord));

			BYTE * pcbByte=(BYTE *)(&pSafeBind->ClientSerial);
			for (INT i=0;i<sizeof(tagClientSerial)/sizeof(BYTE);i++)
			{
				ASSERT(CountArray(SafeBind.szComputerID)>((i*2)+1));
				_stprintf(&SafeBind.szComputerID[i*2],TEXT("%02X"),*(pcbByte+i));
			}

			//投递消息
			m_pIDataBaseEngine->PostDataBaseRequest(DBR_GP_SAFE_BIND, dwSocketID, &SafeBind, sizeof(SafeBind));

			return true;
		}
	case SUB_GP_SAFE_UNBIND:		//解除绑定
		{
			//效验参数
			ASSERT(wDataSize==sizeof(CMD_GP_SafUnbind));
			if (wDataSize!=sizeof(CMD_GP_SafUnbind)) return false;

			//解析变量
			CMD_GP_SafUnbind *pSafUnbind = (CMD_GP_SafUnbind*)pData;

			//设置变量
			DBR_GP_SafeUnbind SafeUnbind;
			ZeroMemory(&SafeUnbind, sizeof(SafeUnbind));
			SafeUnbind.dwUserID = pSafUnbind->dwUserID ;
			lstrcpyn(SafeUnbind.szPassWord,pSafUnbind->szPassWord,CountArray(SafeUnbind.szPassWord));

			//投递消息
			m_pIDataBaseEngine->PostDataBaseRequest(DBR_GP_SAFE_UNBIND, dwSocketID, &SafeUnbind, sizeof(SafeUnbind));
	...
	case SUB_GP_OPEN_MONEY:		//操作保险柜 2011-4-10 新增功能
		{
			//数据验证
			ASSERT(sizeof(CMD_GP_MoneyOper) == wDataSize);
			if ( sizeof(CMD_GP_MoneyOper) != wDataSize) return false;	

			CMD_GP_MoneyOper *pMoneyOper = (CMD_GP_MoneyOper *)pData;

			DBR_GP_OperMoney OperMoney;
			OperMoney.dwUserID = pMoneyOper->dwUserID;
			OperMoney.dwKindID = pMoneyOper->dwKindID;
			OperMoney.cbType = pMoneyOper->cbType;
			OperMoney.lScore = pMoneyOper->lScore;

			if(OperMoney.lScore < 0) return false;

			//投递请求
			m_pIDataBaseEngine->PostDataBaseRequest(DBR_GP_OPER_MONEY, dwSocketID, &OperMoney, sizeof(OperMoney));

			return true;
		}
	case SUB_GP_GIVE_MONEY:		//赠送钱数 2011-4-12 新增功能
		{
			//数据验证
			ASSERT(sizeof(CMD_GP_MoneyGive) == wDataSize);
			if ( sizeof(CMD_GP_MoneyGive) != wDataSize) return false;	

			CMD_GP_MoneyGive *pMoneyOper = (CMD_GP_MoneyGive *)pData;

			DBR_GP_GiveMoney OperMoney;
			OperMoney.dwUserID = pMoneyOper->dwUserID;
			strcpy(OperMoney.szNickname,pMoneyOper->szNickname);
			OperMoney.cbType = pMoneyOper->cbType;
			OperMoney.cbQueryInfo = pMoneyOper->cbQueryInfo;

			if(pMoneyOper->lScore < 0) return false;
			//计算扣税
			LONGLONG lRevenue=pMoneyOper->lScore * 0.02;
			LONGLONG lSwapeScore = pMoneyOper->lScore-lRevenue;
			OperMoney.lRevenue = lRevenue;
			OperMoney.lScore = lSwapeScore;



			//投递请求
			m_pIDataBaseEngine->PostDataBaseRequest(DBR_GP_GIVE_MONEY, dwSocketID, &OperMoney, sizeof(OperMoney));

			break;
		}
	case SUB_GP_EDIT_BANK_PASS:   //修改保险柜密码
		{
			//数据验证
			ASSERT(sizeof(CMD_GP_EditBankPass) == wDataSize);
			if ( sizeof(CMD_GP_EditBankPass) != wDataSize) return false;	

			CMD_GP_EditBankPass *pBankPass = (CMD_GP_EditBankPass *)pData;

			DBR_GP_BankPass BankPass;
			BankPass.dwUserID = pBankPass->dwUserID;
			strcpy(BankPass.szPassword,pBankPass->szPassword);
			strcpy(BankPass.szPassword1,pBankPass->szPassword1);
			
			//投递请求
			m_pIDataBaseEngine->PostDataBaseRequest(DBR_GP_BANK_PASS, dwSocketID, &BankPass, sizeof(BankPass));


			break;
		}
	case SUB_GP_GIVE_RECORD:	  //获取赠送记录
		{
			//数据验证
			ASSERT(sizeof(CMD_GP_BankRecord) == wDataSize);
			if ( sizeof(CMD_GP_BankRecord) != wDataSize) return false;	

			CMD_GP_BankRecord *pGiveRecord = (CMD_GP_BankRecord *)pData;

			DBR_GP_GiveRecord GiveRecord;
			GiveRecord.dwUserID = pGiveRecord->dwUserID;
		
			
			//投递请求
			m_pIDataBaseEngine->PostDataBaseRequest(DBR_GP_GIVE_RECORD, dwSocketID, &GiveRecord, sizeof(GiveRecord));
		...
		case SUB_GP_EDIT_USER_PASS:   //修改保险柜密码
		{
			//数据验证
			ASSERT(sizeof(CMD_GP_ModifyUserPass) == wDataSize);
			if ( sizeof(CMD_GP_ModifyUserPass) != wDataSize) return false;	

			CMD_GP_EditBankPass *pUserPass = (CMD_GP_EditBankPass *)pData;

			DBR_GP_BankPass UserPass;
			UserPass.dwUserID = pUserPass->dwUserID;
			strcpy(UserPass.szPassword,pUserPass->szPassword);
			strcpy(UserPass.szPassword1,pUserPass->szPassword1);
			
			//投递请求
			m_pIDataBaseEngine->PostDataBaseRequest(DBR_GP_USER_PASS, dwSocketID, &UserPass, sizeof(UserPass));


			break;
		}
		case SUB_GP_MODIFY_WRITE:   //修改保险柜密码
		{
			//数据验证
			ASSERT(sizeof(CMD_GP_ModifyUserWrite) == wDataSize);
			if ( sizeof(CMD_GP_ModifyUserWrite) != wDataSize) return false;	

			CMD_GP_ModifyUserWrite *pUserWrite = (CMD_GP_ModifyUserWrite *)pData;

			DBR_GP_UserWrite UserWrite;
			UserWrite.dwUserID = pUserWrite->dwUserID;
			strcpy(UserWrite.szUnderWrite,pUserWrite->szUnderWrite);
			
			
			//投递请求
			m_pIDataBaseEngine->PostDataBaseRequest(DBR_GP_USER_WRITE, dwSocketID, &UserWrite, sizeof(UserWrite));


			break;
		}
		case SUB_GP_EDIT_USER_INFO:   //用户详细信息
		{
			//数据验证
			ASSERT(sizeof(CMD_GP_ModifyUserInfo) == wDataSize);
			if ( sizeof(CMD_GP_ModifyUserInfo) != wDataSize) return false;	

			CMD_GP_ModifyUserInfo *pUserInfo = (CMD_GP_ModifyUserInfo *)pData;

			DBR_GP_ModifyUserInfo UserInfo;
			UserInfo.dwUserID = pUserInfo->dwUserID;
			strcpy(UserInfo.szAddress,pUserInfo->szAddress);
			strcpy(UserInfo.szEmail,pUserInfo->szEmail);
			strcpy(UserInfo.szMsn,pUserInfo->szMsn);
			strcpy(UserInfo.szSchool,pUserInfo->szSchool);
			strcpy(UserInfo.szTel,pUserInfo->szTel);
			strcpy(UserInfo.szUrl,pUserInfo->szUrl);
			
			
			//投递请求
			m_pIDataBaseEngine->PostDataBaseRequest(DBR_GP_USER_INFO, dwSocketID, &UserInfo, sizeof(UserInfo));
...
//解除绑定
bool CAttemperEngineSink::OnDBOperMoney(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//合法判断
	ASSERT(LOWORD(dwContextID)<m_pInitParamter->m_wMaxConnect);
	if ((m_pBindParameter+LOWORD(dwContextID))->dwSocketID!=dwContextID) return true;

	DBR_GP_MoneyOperResult *pMoneyOperResult=(DBR_GP_MoneyOperResult *)pData;

	CMD_GP_MoneyOperResult MoneyOperResult;
	ZeroMemory(&MoneyOperResult, sizeof(MoneyOperResult));

	lstrcpyn(MoneyOperResult.szDescribeMsg,pMoneyOperResult->szDescribeMsg , sizeof(MoneyOperResult.szDescribeMsg));
	MoneyOperResult.bOperateSuccess = pMoneyOperResult->bOperateSuccess;


	//发送消息
	m_pITCPNetworkEngine->SendData(dwContextID,MDM_GP_USER,SUB_GP_OPEN_MONEY,&MoneyOperResult,sizeof(MoneyOperResult));
...
//解除绑定
bool CAttemperEngineSink::OnDBRefreshMoney(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//合法判断
	ASSERT(LOWORD(dwContextID)<m_pInitParamter->m_wMaxConnect);
	if ((m_pBindParameter+LOWORD(dwContextID))->dwSocketID!=dwContextID) return true;

	DBR_GP_MoneyRefreshResult *pMoneyOperResult=(DBR_GP_MoneyRefreshResult *)pData;

	CMD_GP_MoneyRefreshResult MoneyOperResult;
	ZeroMemory(&MoneyOperResult, sizeof(MoneyOperResult));

	MoneyOperResult.lTotalScore = pMoneyOperResult->lTotalScore;
	MoneyOperResult.lRoomScore = pMoneyOperResult->lRoomScore;


	//发送消息
	m_pITCPNetworkEngine->SendData(dwContextID,MDM_GP_USER,SUB_GP_REFRESH_MONEY,&MoneyOperResult,sizeof(MoneyOperResult));
...
//////////////////////////////////////////////////////////////////////////
//赠送操作结果 2012-4-12
bool CAttemperEngineSink::OnDBGiveMoney(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//合法判断
	ASSERT(LOWORD(dwContextID)<m_pInitParamter->m_wMaxConnect);
	if ((m_pBindParameter+LOWORD(dwContextID))->dwSocketID!=dwContextID) return true;

	DBR_GP_GiveMoneyResult *pMoneyOperResult=(DBR_GP_GiveMoneyResult *)pData;

	CMD_GP_MoneyGiveResult MoneyOperResult;
	ZeroMemory(&MoneyOperResult, sizeof(MoneyOperResult));
	
	lstrcpyn(MoneyOperResult.szDescribeMsg,pMoneyOperResult->szDescribeMsg , sizeof(MoneyOperResult.szDescribeMsg));
	lstrcpyn(MoneyOperResult.szTargeAccount,pMoneyOperResult->szTargeAccount, sizeof(MoneyOperResult.szTargeAccount));
	MoneyOperResult.bQueryInfo = pMoneyOperResult->bQueryInfo;
	MoneyOperResult.lSwapScore = pMoneyOperResult->lSwapScore;
	MoneyOperResult.dwTargetUserId = pMoneyOperResult->dwTargetUserId;
	MoneyOperResult.bOperateSuccess = pMoneyOperResult->bOperateSuccess;

	//发送消息
	m_pITCPNetworkEngine->SendData(dwContextID,MDM_GP_USER,SUB_GP_GIVE_MONEY,&MoneyOperResult,sizeof(MoneyOperResult));
...
//赠送操作结果 2012-4-12
bool CAttemperEngineSink::OnDBBankPass(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//合法判断
	ASSERT(LOWORD(dwContextID)<m_pInitParamter->m_wMaxConnect);
	if ((m_pBindParameter+LOWORD(dwContextID))->dwSocketID!=dwContextID) return true;

	DBR_GP_BankPassResult *pMoneyOperResult=(DBR_GP_BankPassResult *)pData;

	CMD_GP_EditBankPassResult MoneyOperResult;
	ZeroMemory(&MoneyOperResult, sizeof(MoneyOperResult));
	...
	//发送消息
	m_pITCPNetworkEngine->SendData(dwContextID,MDM_GP_USER,SUB_GP_EDIT_BANK_PASS,&MoneyOperResult,sizeof(MoneyOperResult));
...
//赠送操作结果 2012-4-12
bool CAttemperEngineSink::OnDBGiveRecord(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//合法判断
	ASSERT(LOWORD(dwContextID)<m_pInitParamter->m_wMaxConnect);
	if ((m_pBindParameter+LOWORD(dwContextID))->dwSocketID!=dwContextID) return true;

	DBR_GP_GiveRecordResult *pMoneyOperResult=(DBR_GP_GiveRecordResult *)pData;

	CMD_GP_BankRecordResult MoneyOperResult;
	ZeroMemory(&MoneyOperResult, sizeof(MoneyOperResult));
	
	lstrcpyn(MoneyOperResult.szDescribeMsg,pMoneyOperResult->szDescribeMsg , sizeof(MoneyOperResult.szDescribeMsg));
	MoneyOperResult.bOperateSuccess = pMoneyOperResult->bOperateSuccess;
	MoneyOperResult.nCount = pMoneyOperResult->nCount;
	memcpy(MoneyOperResult.GiveRecord,pMoneyOperResult->GiveRecord,MoneyOperResult.nCount*sizeof(tagUserGiveRecord));


	//发送消息
	m_pITCPNetworkEngine->SendData(dwContextID,MDM_GP_USER,SUB_GP_GIVE_RECORD,&MoneyOperResult,sizeof(MoneyOperResult)-(200-MoneyOperResult.nCount)*sizeof(tagUserGiveRecord));
