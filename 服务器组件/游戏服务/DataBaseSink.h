...
#define DBR_GR_BANK_GET_GOLD			13								//提取欢乐豆
#define DBR_GR_BANK_STORAGE_GOLD		14								//存储欢乐豆
#define DBR_GR_BANK_TRANSFER            15                              //转出欢乐豆
#define DBR_GR_BANK_SECURITY            16                              //密码安全

#define DBR_GR_WRITESCORE               17                             //每局结束后的分数存储
//#define DBR_GR_USER_SIGN_UP				12								//玩家报名
#define DBR_GR_BANK_RESULT              30                              //银行结果
...
#define DBR_GR_WRITER_SCORE  			111								//写分后机器人自动加钱的显示
#define DBR_GR_WRITER_SCOREJIQI  			112								//写分后机器人自动加钱的显示
//赠送鲜花
...
struct DBR_GR_RecordGoldLog
{
int dwGameID;			//用户游戏ID
int dwKindID;			//游戏ID
int dwServerID;		    // 房间ID
LONGLONG wRevenue;          //
LONGLONG lSwapScore;    // 变动欢乐豆数
};
struct DBR_GR_SendWriterScroe
{
 DWORD GameUserID;
 LONGLONG UserScore;
};
...
//转出欢乐豆
struct DBR_GR_BankTransfer
{
	DWORD                               dwUserID;                       //用户 I D
	DWORD                               dwTagGameID;                    //目标用户
	LONGLONG                            lTransferCount;                 //转出数目
	LONGLONG                            lRevenue;	                    //扣取税收
	LONGLONG							lCurrentScore;
	LONGLONG							lCurrentBankScore;
	DWORD                               dwClientIP;                     //用户 I P
	BYTE								cbGameAction;					//游戏操作
	BYTE								cbUserOneline;					//用户在线
	
};

//银行安全
struct DBR_GR_BankSecurity
{
	DWORD								dwUserID;						//用户 I D
	DWORD                               dwClientIP;                     //用户 I P
	TCHAR                               szNewPassword[PASS_LEN];        //银行密码
};

struct DBR_GR_BankResult
{
	BYTE								cbGameAction;					//游戏操作
	BYTE								cbUserOnline;					//用户在线
	DWORD                               dwUserID;                       //用户 I D
	DWORD                               dwTagGameID;                    //目标用户
	LONGLONG                            lModifyGold;                    //修改欢乐豆
	LONGLONG							lRevenue;						//转账税收
	LONG                                lResultType;				    //操作类型
...
struct DBR_GR_LogonSuccess
{
	...
	LONGLONG 							lInsureScore;					//存储欢乐豆
	LONGLONG 							lGameGold;						//游戏欢乐豆
	LONG								lWinCount;						//胜利盘数
...
private:
	...
	bool OnBankStorageGold(DWORD dwContextID, VOID * pData, WORD wDataSize);
		//转出欢乐豆
	bool OnBankTransferGold(DWORD dwContextID, VOID * pData, WORD wDataSize);
	//密码安全
	bool OnBankSecurity(DWORD dwContextID, VOID * pData, WORD wDataSize);
	//每局的结束玩家写入欢乐豆
    bool WriteScore(DWORD dwContextID, VOID * pData, WORD wDataSize);
	//存储过程
protected:
	//I D 存储过程
	LONG SPLogonByUserID(DWORD dwUserID, LPCTSTR pszPassword, DWORD dwClientIP, LPCTSTR pszComputerID);
	//写分存储过程
	LONG SPWriteUserScore(DWORD dwUserID, DWORD dwPlayTimeCount, DWORD dwOnLineTimeCount, DWORD dwClientIP, LONGLONG lRevenue, tagUserScore & UserScore);
	//离开存储过程
	LONG SPLeaveGameServer(DWORD dwUserID, DWORD dwPlayTimeCount, DWORD dwOnLineTimeCount, DWORD dwClientIP, LONGLONG lRevenue, LONG lLoveliness, tagUserScore & UserScore);
	//禁号存储过程
	...
	LONG SPBankStorageGold(DWORD dwUserID, LONGLONG lSwapScore, DWORD dwClientIP);
		//转帐存储过程
	LONG SPBankTransferGold(DWORD dwUserID, DWORD dwTagGameID, LONGLONG lSwapScore, LONGLONG lRevenue,
		LONGLONG lCurrentScore, LONGLONG lCurrentBankScore, BYTE cbUserOnline, DWORD dwClientIP);
	//更新密码过程
	LONG SPBankSecurity(DWORD dwUserID, LPCTSTR pszPassword, DWORD dwClientIP);
};
