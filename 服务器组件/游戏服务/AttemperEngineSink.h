...
protected:
	...
	bool							m_bAllowEnterGame;					//进入标志
		...
		WORD							m_wRootMaxFree;						//最大人数
	WORD							m_wRootMinFree;						//最小人数
	//配置信息
protected:
...
private:
	...
	bool OnBankResult(DWORD dwContextID, VOID * pData, WORD wDataSize);
	//写入积分
    bool OnWriteScore(DWORD dwContextID, VOID * pData, WORD wDataSize);
	bool OnWriteScore1(DWORD dwContextID, VOID * pData, WORD wDataSize);
	//网络函数
private:
	//登录消息处理
...
protected:
	...
	////安全事件
	bool OnEventBankSecurity(const void * pData, WORD wDataSize, DWORD dwSocketID);
