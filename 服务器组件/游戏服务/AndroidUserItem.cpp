...
// 	while (true)
// 	{
// 		pIServerUserItem=m_pIServerUserManager->EnumOnLineUser(wEnumIndex++);
// 		if (pIServerUserItem==NULL) break;
// 		if (pIServerUserItem->GetTableID()==m_pIServerUserItem->GetTableID()) break;
// 	}
// 	//获取同桌的人的信息。。只有对家啊。。。。。不能反回多个值 啊。
// 
...
bool __cdecl CAndroidUserItem::SendMainUser( WORD wSubCmdId, VOID* pData, WORD wDataSize )
{
	//状态效验
	ASSERT((m_pIServerUserItem!=NULL)&&(m_pIServerUserItem->GetUserStatus()==US_SIT));
	if ((m_pIServerUserItem==NULL)||(m_pIServerUserItem->GetUserStatus()!=US_SIT)) return false;
	...
	//发送准备
	DWORD dwAndroidID=MAKELONG(m_wAndroidIndex,m_wRountID);
	m_pIAndroidUserManager->SendDataToServer(dwAndroidID,MDM_GR_USER,wSubCmdId,pData,wDataSize);
	return true;
}
