...
{
	//启动时间
	m_pITimerEngine->SetTimer(IDI_ANDROID_INOUT,3/*TIME_ANDROID_INOUT*/*1000L,TIMES_INFINITY,0);
...
bool __cdecl CAndroidUserManager::DeleteAndroidUserItem(DWORD dwAndroidID)
{
	...
	//对象效验
	WORD wRoundID=HIWORD(dwAndroidID);
	...
	case IDI_ANDROID_INOUT:			//进出处理
		{
			...
			_snscanf(OutBuf,strlen(OutBuf),"%I64d",&m_lUserMaxScore);
			memset(OutBuf,0,255);
			GetPrivateProfileString(m_pGameServiceOption->szGameRoomName,TEXT("AndroidMinScore"),"10000",OutBuf,255,szIniFileName);
			_snscanf(OutBuf,strlen(OutBuf),"%I64d",&m_lUserMinScore);
			m_wAutoAndroidCount = GetPrivateProfileInt(m_pGameServiceOption->szGameRoomName, TEXT("AutoAndroidCount"), 128, szIniFileName);
			...
			while (Position!=NULL)
			{
				...
				if (pAndroidUserItem->m_pIServerUserItem!=NULL)
				{
					...
					if(nActiveCount <= m_wAutoAndroidCount)
					{
						if(rand()%60 !=0) continue;
						//状态判断
						if (dwOnlineTimeCount<=(TIME_ANDROID_SERVICE+rand()%200)) continue;
					}
