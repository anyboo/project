...
bool __cdecl CDataBaseSink::OnDataBaseEngineRequest(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	case DBR_GR_BANK_GET_GOLD:			//提取欢乐豆
		{
			return OnBankDrawoutGold(dwContextID,pData,wDataSize);
		}
	case DBR_GR_BANK_STORAGE_GOLD:		//存储欢乐豆
		{
			return OnBankStorageGold(dwContextID,pData,wDataSize);
		}
    case DBR_GR_BANK_TRANSFER:			//转出欢乐豆
		{
			return OnBankTransferGold(dwContextID,pData,wDataSize);
		}
	case DBR_GR_BANK_SECURITY:			//密码安全
		{
			return OnBankSecurity(dwContextID,pData,wDataSize);
		}
	case DBR_GR_WRITESCORE:
		{
		  return  WriteScore(dwContextID,pData,wDataSize);
		}
	}
...
//转出欢乐豆
bool CDataBaseSink::OnBankTransferGold(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	ASSERT(wDataSize==sizeof(DBR_GR_BankTransfer));
	if (wDataSize!=sizeof(DBR_GR_BankTransfer)) return false;

	DBR_GR_BankTransfer *pBankTransfer=(DBR_GR_BankTransfer *)pData;
	...
	{
		//提取欢乐豆
		lReturnValue = SPBankTransferGold(pBankTransfer->dwUserID,
			pBankTransfer->dwTagGameID,
			pBankTransfer->lTransferCount,
			pBankTransfer->lRevenue,
			pBankTransfer->lCurrentScore,
			pBankTransfer->lCurrentBankScore,
			pBankTransfer->cbUserOneline,
			pBankTransfer->dwClientIP);

		DBR_GR_BankResult BankResult;
		ZeroMemory(&BankResult,sizeof(BankResult));
		BankResult.cbGameAction=pBankTransfer->cbGameAction;
		BankResult.cbUserOnline=pBankTransfer->cbUserOneline;
		BankResult.dwUserID=pBankTransfer->dwUserID;
		BankResult.lModifyGold=pBankTransfer->lTransferCount;
		BankResult.lRevenue = pBankTransfer->lRevenue;
		BankResult.lResultType=3;
		BankResult.lErrorCode=lReturnValue;
		BankResult.dwTagGameID = pBankTransfer->dwTagGameID;

//		if (lReturnValue!=0)
//			m_GameScoreDBAide.GetValue_String(TEXT("ErrorDescribe"),BankResult.szErrorDescribe,CountArray(BankResult.szErrorDescribe));

		m_GameScoreDBModule->CloseRecordset();
		m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBR_GR_BANK_RESULT,dwContextID,&BankResult,sizeof(BankResult));
		return true;
	}
	catch (IDataBaseException * pIException)
	{
...
//密码安全
bool CDataBaseSink::OnBankSecurity(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	ASSERT(wDataSize==sizeof(DBR_GR_BankSecurity));
	if (wDataSize!=sizeof(DBR_GR_BankSecurity)) return false;

	DBR_GR_BankSecurity *pBankSecurity=(DBR_GR_BankSecurity *)pData;
	ASSERT(pBankSecurity->dwUserID!=0);

	LONG lReturnValue = 0;

	try
	{
		lReturnValue=SPBankSecurity(pBankSecurity->dwUserID,pBankSecurity->szNewPassword,pBankSecurity->dwClientIP);
	}
	catch (IDataBaseException * pIException)
	{
		//错误信息
...
bool CDataBaseSink::OnRequestLogon(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	try
	{
		...
		m_GameScoreDBAide.GetValue_String(TEXT("Accounts"),LogonSuccess.szGroupName,CountArray(LogonSuccess.szGroupName));
	...
	catch (IDataBaseException * pIException)
	{
		...
		lstrcpyn(LogonError.szErrorDescribe,TEXT("由于数据库操作异常，请您稍后重试或选择另一游戏服务器！"),CountArray(LogonError.szErrorDescribe));
		m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBR_GR_LOGON_ERROR,dwContextID,&LogonError,sizeof(LogonError));
	}
...
bool CDataBaseSink::OnRequestLoadAndroid(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		...
		if (m_pGameServiceOption->wServerType == GAME_GENRE_GOLD)
		{
			...
			lstrcat(szIniFileName, TEXT("\\ServiceLoader.ini"));

			LONGLONG lMaxScore,lMinScore;

			TCHAR OutBuf[255];
			memset(OutBuf,0,255);
			GetPrivateProfileString(m_pGameServiceOption->szGameRoomName,TEXT("AndroidMaxScore"),"2000000000000000",OutBuf,255,szIniFileName);
			_snscanf(OutBuf,strlen(OutBuf),"%I64d",&lMaxScore);
			memset(OutBuf,0,255);
			GetPrivateProfileString(m_pGameServiceOption->szGameRoomName,TEXT("AndroidMinScore"),"100",OutBuf,255,szIniFileName);
			_snscanf(OutBuf,strlen(OutBuf),"%I64d",&lMinScore);

			m_GameScoreDBAide.AddParameter(TEXT("@lMaxScore"), lMaxScore);
			m_GameScoreDBAide.AddParameter(TEXT("@lMinScore"), lMinScore);	
		}
...
bool CDataBaseSink::WriteScore(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
  
	 ASSERT(wDataSize==sizeof(DBR_GR_RecordGoldLog));
	if (wDataSize!=sizeof(DBR_GR_RecordGoldLog)) 
	{
		//MessageBox(NULL,"师傅哈市7777","asdasd",NULL);
		return false;
	}
    
	DBR_GR_RecordGoldLog * pBankStorage=(DBR_GR_RecordGoldLog *)pData;

	m_GameScoreDBAide.ResetParameter();
	m_GameScoreDBAide.AddParameter(TEXT("@dwGameID"),pBankStorage->dwGameID);
	m_GameScoreDBAide.AddParameter(TEXT("@dwKindID"),pBankStorage->dwKindID);
	m_GameScoreDBAide.AddParameter(TEXT("@dwServerID"),pBankStorage->dwServerID);
	m_GameScoreDBAide.AddParameter(TEXT("@lSwapScore"),pBankStorage->lSwapScore);
	m_GameScoreDBAide.AddParameter(TEXT("@Revenue"),pBankStorage->wRevenue);
   
	//MessageBox(NULL,"师傅哈市66666","asdasd",NULL);
  LONG lReturnValue= m_GameScoreDBAide.ExecuteProcess(TEXT("GSP_GR_RecordGoldLog"),true);
   // return m_GameScoreDBAide.ExecuteProcess(TEXT("GSP_GR_RecordGoldLog"),false);

  if(lReturnValue==1)
  {

	DBR_GR_SendWriterScroe ModifyIndividualResult;
	ZeroMemory(&ModifyIndividualResult, sizeof(ModifyIndividualResult));
    
	
	ModifyIndividualResult.GameUserID=m_GameScoreDBAide.GetValue_DWORD(TEXT("UserID"));

	
	ModifyIndividualResult.UserScore=m_GameScoreDBAide.GetValue_LONGLONG(TEXT("Score"));//锁定机器
    

	m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBR_GR_WRITER_SCORE,dwContextID, &ModifyIndividualResult, sizeof(ModifyIndividualResult));
  ...
  }else if(lReturnValue==2)
  {
	...
	m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBR_GR_WRITER_SCOREJIQI,dwContextID, &ModifyIndividualResult, sizeof(ModifyIndividualResult));
...
//写分存储过程
LONG CDataBaseSink::SPWriteUserScore(DWORD dwUserID, DWORD dwPlayTimeCount, DWORD dwOnLineTimeCount, DWORD dwClientIP, LONGLONG lRevenue, tagUserScore & UserScore)
{
	//效验参数
	ASSERT(dwUserID!=0L);
...
//离开存储过程
LONG CDataBaseSink::SPLeaveGameServer(DWORD dwUserID, DWORD dwPlayTimeCount, DWORD dwOnLineTimeCount, DWORD dwClientIP, LONGLONG lRevenue, LONG lLoveliness, tagUserScore & UserScore)
{
	//效验参数
	ASSERT(dwUserID!=0L);
...
//转帐存储过程
LONG CDataBaseSink::SPBankTransferGold(DWORD dwUserID, DWORD dwTagGameID, LONGLONG lSwapScore, LONGLONG lRevenue,
									   LONGLONG lCurrentScore, LONGLONG lCurrentBankScore, BYTE cbUserOnline, DWORD dwClientIP)
{
	ASSERT( dwUserID != 0 && dwTagGameID != 0);
	...
	m_GameScoreDBAide.ResetParameter();
	m_GameScoreDBAide.AddParameter(TEXT("@dwUserID"),dwUserID);
	m_GameScoreDBAide.AddParameter(TEXT("@dwTagGameID"),dwTagGameID);
	m_GameScoreDBAide.AddParameter(TEXT("@lSwapScore"),lSwapScore);
	m_GameScoreDBAide.AddParameter(TEXT("@lRevenue"),lRevenue);
	m_GameScoreDBAide.AddParameter(TEXT("@lCurrentScore"),lCurrentScore);
	m_GameScoreDBAide.AddParameter(TEXT("@lCurrentBankScore"),lCurrentBankScore);
	m_GameScoreDBAide.AddParameter(TEXT("@bIsOnline"),cbUserOnline);
	m_GameScoreDBAide.AddParameter(TEXT("@dwKindID"),m_pGameServiceOption->wKindID);
	m_GameScoreDBAide.AddParameter(TEXT("@dwServerID"),m_pGameServiceOption->wServerID);
	m_GameScoreDBAide.AddParameter(TEXT("@strClientIP"),szClientIP);
	m_GameScoreDBAide.AddParameter(TEXT("@bIsGameIDTransfer"),1);
	m_GameScoreDBAide.AddParameter(TEXT("@strClientIP"),TEXT("11111"));

	return m_GameScoreDBAide.ExecuteProcess(TEXT("GSP_GR_TransferGameGold"),true);
}

//更新密码过程
LONG CDataBaseSink::SPBankSecurity(DWORD dwUserID, LPCTSTR pszPassword, DWORD dwClientIP)
{
	ASSERT(dwUserID!=0);
	...
	m_AccountsDBAide.ResetParameter();
	m_AccountsDBAide.AddParameter(TEXT("@dwUserID"),dwUserID);
	m_AccountsDBAide.AddParameter(TEXT("@strPassowrd"),pszPassword);
	m_AccountsDBAide.AddParameter(TEXT("@dwKindID"),m_pGameServiceOption->wKindID);
	m_AccountsDBAide.AddParameter(TEXT("@dwServerID"),m_pGameServiceOption->wServerID);
	m_AccountsDBAide.AddParameter(TEXT("@strClientIP"),szClientIP);

	return m_AccountsDBAide.ExecuteProcess(TEXT("GSP_GR_ModifyPassword"),false);
}
