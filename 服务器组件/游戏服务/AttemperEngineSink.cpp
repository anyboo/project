...
bool __cdecl CAttemperEngineSink::OnAttemperEngineStart(IUnknownEx * pIUnknownEx)
{
	...
	case GAME_GENRE_GOLD:	//欢乐豆类型
		{
			...
			tagColumnItem ColumnItem[]=
			{
				...
				20,1001,TEXT(" "),
				0,1002,TEXT(" "),
				85,DTP_USER_ACCOUNTS,TEXT("昵称"),
					40,DTP_GAME_ID,TEXT("ID"),
					80,DTP_USER_SCORE,TEXT("欢乐豆"),
					40,DTP_USER_TABLE,TEXT("桌号"),
					0,DTP_LOVE_LINESS,TEXT("魅力"),
	...
	if(m_pGameServiceOption->wTableCount!=1)
		m_pITimerEngine->SetTimer(IDI_DISTRIBUTE_ANDROID,1*1000L,(DWORD)(-1),NULL);
	else
		m_pITimerEngine->SetTimer(IDI_DISTRIBUTE_ANDROID,2*1000L,(DWORD)(-1),NULL);

	//设置时间
	m_pITimerEngine->SetTimer(IDI_LOAD_ANDROID_INFO,/*TIME_LOAD_ANDROID_INFO*/10*1000L,(DWORD)(-1),NULL);
	//////////////////////////////////////////////////////////////////////////
		...
		case IDI_DISTRIBUTE_ANDROID:	//分配用户
			{
				...
				for(WORD wAndroidUser=0; wAndroidUser<AndroidUserStatus.wFreeUserCount; ++wAndroidUser)
				{
					...
					//积分范围
					LONGLONG lUserScore = pIServerUserItem->GetUserData()->UserScoreInfo.lScore ;
					if(lUserScore >=m_AndroidUserManager.m_lUserMinScore && lUserScore <= m_AndroidUserManager.m_lUserMaxScore) 
						continue;
...
// 							//		if ((pIServerUserItem->GetUserData()->dwUserRight&UR_ROROT_NOEXIT) == 0) continue;
...
// 							if (m_pGameServiceAttrib->wChairCount<MAX_CHAIR_NORMAL) 
// 							{
// 								if (wUserSitCount==0) continue;  //机器人+真人数目
// 								//	if (TableUserStatus.wTableUserCount==0) continue; //真人数目
// 
// 							}
// 
// 							//最低限制
// 							int nRootFree=0;//(rand()%(m_wRootMinFree+1))+(m_wRootMaxFree-m_wRootMinFree);
// 
// 							//if (m_pGameServiceAttrib->wChairCount>MAX_CHAIR_NORMAL
// 							//	&&AndroidUserStatus.wFreeUserCount<nRootFree) continue;//空闲机器人数目
// 
// 
// 							//by
// 							if (m_pGameServiceAttrib->wChairCount>MAX_CHAIR_NORMAL
// 								&&AndroidUserStatus.wFreeUserCount<nRootFree
// 								&&wAndroidCount>nRootFree) continue;//空闲机器人数目
// 
...
// 									break;
// 								WORD j=0;
// 								j=rand()%AndroidUserStatus.wFreeUserCount;
// 								pIServerUserItem=AndroidUserStatus.pServerUserFree[j];
...
// 							if (rand()%100>30)
// 								pTableFrame=(m_pTableFrame+(INT_PTR)(rand()%m_pGameServiceOption->wTableCount)/5);
// 							else if (rand()%100>60)
// 								pTableFrame=(m_pTableFrame+(INT_PTR)(rand()%m_pGameServiceOption->wTableCount)/2);
// 							else if (rand()%100>90)
// 								pTableFrame=(m_pTableFrame+(INT_PTR)(rand()%m_pGameServiceOption->wTableCount));
// 
// 
...
// 					}
// 					//非百人起立处理
// 					if (AndroidUserStatus.wSitdownUserCount>0&&m_pGameServiceAttrib->wChairCount<=MAX_CHAIR_NORMAL)
// 					{
// 						for (INT_PTR i=0;i<m_pGameServiceOption->wTableCount;i++)
// 						{
// 							//获取桌子
...
// 								if (pIServerUserItem==NULL) continue;
// 								if(!pIServerUserItem->IsAndroidUser()) continue;
// 								//tagAndroidOut Android=pIServerUserItem->GetAndroidOut();
// 								//const tagUserScore *pUserSocre=pIServerUserItem->GetUserScore();
// 								//LONG lPlayCount=pUserSocre->lFleeCount+pUserSocre->lDrawCount+pUserSocre->lLostCount+pUserSocre->lWinCount;
// 								//if (lPlayCount>=Android.wRobotStandUp)
// 								if (rand()%5==0)													
// 								{
// 									pTableFrame->PerformStandUpAction(pIServerUserItem);
// 								}
// 								//只有一个准备机器人 起立
// 								if (TableUserStatus.wTableAndroidCount==1&&TableUserStatus.wTableUserCount==0&&rand()%3==0)
// 								{
// 									pTableFrame->PerformStandUpAction(pIServerUserItem);
// 								}
// 							}
...
// 							{
// 								WORD j=0;
// 								j=rand()%pTableFrame->GetChairCount();
// 								IServerUserItem * pIServerUserItem=pTableFrame->GetServerUserItem(j);
// 								if (pIServerUserItem==NULL) continue;
// 								if(!pIServerUserItem->IsAndroidUser()) continue;
...
// 								//by
// 								//if ((pIServerUserItem->GetUserData()->dwUserRight&UR_ROROT_NOEXIT) != 0) continue;
// 								if ((pIServerUserItem->GetUserData()->dwUserRight&UR_ROROT_NOEXIT) != 0) return true;
// 
// 								if (pTableFrame->PerformStandUpAction(pIServerUserItem)==true) break;
// 							}
// 						}
// 					}
// 				}

				wAndroidCount=GetAndroidUserStatus(AndroidUserStatus);
				wAndroidBusyCount=AndroidUserStatus.wPlayUserCount+AndroidUserStatus.wSitdownUserCount;
				srand(time(NULL));
				//动作处理
				if (wAndroidCount>0)
				{
					IServerUserItem * pISitUserItem = NULL;				//是不是同一个桌子 
					//分析变量
					tagTableUserStatus TableUserStatus;
					ZeroMemory(&TableUserStatus,sizeof(TableUserStatus));
					...
					if (AndroidUserStatus.wFreeUserCount>0)
					{
						WORD wTableCount = m_pGameServiceOption->wTableCount>7?m_pGameServiceOption->wTableCount-6:m_pGameServiceOption->wTableCount;
						for (INT_PTR i=0;i<wTableCount;i++)
						{
							//获取桌子
							CTableFrame * pTableFrame;
							...
							IServerUserItem * pIServerUserItem=AndroidUserStatus.pServerUserFree[rand()%AndroidUserStatus.wFreeUserCount];
							if (pTableFrame->PerformSitDownAction(wChairID,pIServerUserItem,NULL)==true) {
								pISitUserItem = pIServerUserItem;
								break ;
							}
						...
						//全部机器人
						if(m_pGameServiceAttrib->wChairCount > MAX_CHAIR_NORMAL || (wAndroidBusyCount < (wAndroidCount*9)/10 && m_pGameServiceAttrib->wChairCount <= AndroidUserStatus.wFreeUserCount) )
						{
							bool bSitSuccess = false ;
							//0823ygs机器人桌子查找
							for (INT_PTR i=0;i<wTableCount;i++)
							{
								//获取桌子
								CTableFrame * pTableFrame;
								int nRand = (int)((float)(wTableCount-i) * rand() / (RAND_MAX+1.0));
								if(wTableCount == 1)
									pTableFrame=(m_pTableFrame+i);
								else
									pTableFrame=(m_pTableFrame+nRand);
								...
								for(BYTE wChairIndex=0; wChairIndex<pTableFrame->GetChairCount(); wChairIndex++)
								{
									...
									for(WORD wAndroidUser=0; wAndroidUser<AndroidUserStatus.wFreeUserCount; wAndroidUser++)
									{
										...
										//坐下处理
										if (pTableFrame->PerformSitDownAction(wChairID,pIServerUserItem,NULL)==true) {bIsUserSit = true; break ;}
									}
									if(bIsUserSit) break;
								}
					...
					if (AndroidUserStatus.wSitdownUserCount>0)
					{
						...
						for (INT_PTR i=0;i<wTableCount;i++)
						{
							...
							//用户过虑
							if (TableUserStatus.wTableUserCount>0) 
								continue;
							if (TableUserStatus.wTableAndroidCount==0) continue;
							...
							for (WORD j=0;j<pTableFrame->GetChairCount();j++)
							{
								...
								//只有一个准备机器人 起立
								if (TableUserStatus.wTableAndroidCount==1&&TableUserStatus.wTableUserCount==0&&rand()%10==0)
								{
									pTableFrame->PerformStandUpAction(pIServerUserItem);
									break;
								}
						...
						for (INT_PTR i=0;i<wTableCount;i++)
						{
							...
							for (WORD j=0;j<pTableFrame->GetChairCount();j++)
							{
								...
								if(pIServerUserItem == pISitUserItem) continue;

								//机器人状态
								if(pIServerUserItem->GetUserStatus()>US_READY&& rand()%500 != 0) continue ;		//逃跑
								if(pIServerUserItem->GetUserStatus()==US_READY && rand()%100 != 0) continue;		//准备转台
								if(pIServerUserItem->GetUserStatus()<US_READY && rand()%20 != 0) continue;
					...
					if (m_pGameServiceAttrib->wChairCount>MAX_CHAIR_NORMAL) 
					{
						for (INT_PTR i=0;i<m_pGameServiceOption->wTableCount;i++)
						{
							...
							WORD j=0;
							//起立处理
							while (++j<pTableFrame->GetChairCount())
							{
								IServerUserItem * pIServerUserItem=pTableFrame->GetServerUserItem(j);
								if (pIServerUserItem==NULL) continue;
								if(!pIServerUserItem->IsAndroidUser()) continue;
								...
								//积分范围
								LONGLONG lUserScore = pIServerUserItem->GetUserData()->UserScoreInfo.lScore ;
								if( lUserScore >=m_AndroidUserManager.m_lUserMinScore && lUserScore <= m_AndroidUserManager.m_lUserMaxScore && rand()%100!=0) 
								{
									continue;
								}

								if(pTableFrame->IsGameStarted()== true && rand()%50 != 0) continue; 
...
bool __cdecl CAttemperEngineSink::OnEventDataBase(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	//	}
	case DBR_GR_WRITER_SCORE:	//兑换结果
		{
			return OnWriteScore(dwContextID,pData,wDataSize);
		}
	case DBR_GR_WRITER_SCOREJIQI:
		{
		return OnWriteScore1(dwContextID,pData,wDataSize);
		}
	...
	case DBR_GR_BANK_RESULT:
		{
			return OnBankResult(dwContextID, pData, wDataSize);
		}
...
bool CAttemperEngineSink::OnDBLogonSuccess(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	if((pLogonSuccess->lInsureScore+pLogonSuccess->lScore)<m_pGameServiceOption->lLessScore)
	{
	//发送消息
		TCHAR szDescribe[256]=TEXT("");
				_snprintf(szDescribe,sizeof(szDescribe),TEXT("抱歉，进入本游戏房间至少需要 %ld 的欢乐豆，您的欢乐豆不够，不能进入！"),m_pGameServiceOption->lLessScore);
		SendRoomMessage(dwContextID,szDescribe,SMT_EJECT|SMT_INFO|SMT_INTERMIT_LINE|SMT_CLOSE_ROOM);
	...
	//if(wIndex >= INDEX_ANDROID)
	//	ServerUserData.cbAndroid = true;						//add 0506ygs 机器人标识
	...
	{
		LPCTSTR pszMessage=TEXT("请注意，这是游戏练习房间，您将得到一定数目的模拟欢乐豆，游戏成绩将不会记录到数据库！");
...
bool CAttemperEngineSink::OnDBAndroidUser(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	////设置时间
	m_pITimerEngine->SetTimer(IDI_LOAD_ANDROID_INFO,TIME_LOAD_ANDROID_INFO*1000L,1,NULL); //edit 2012-6-3
...
bool CAttemperEngineSink::OnWriteScore1(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	ASSERT(wDataSize==sizeof(DBR_GR_SendWriterScroe));
	if (wDataSize!=sizeof(DBR_GR_SendWriterScroe)) return false;
    
	DBR_GR_SendWriterScroe * pUserProperty=(DBR_GR_SendWriterScroe *)pData;
    
	IServerUserItem *pServerUserItem = m_ServerUserManager.SearchOnLineUser(pUserProperty->GameUserID);
	if ( pServerUserItem == NULL ) pServerUserItem = m_ServerUserManager.SearchOffLineUser(pUserProperty->GameUserID);//如果查询不到 就在这里在查找一下。
	...
	pUserData->UserScoreInfo.lGameGold=pUserProperty->UserScore;
	pUserData->UserScoreInfo.lScore=pUserProperty->UserScore;
	...
	CTableFrame * pTableFrame=(m_pTableFrame+pUserData->wTableID);
				if (pTableFrame->IsUserPlaying(pServerUserItem)==true)
				{
				   return true;
				}
				pTableFrame->PerformStandUpAction(pServerUserItem);
...
bool CAttemperEngineSink::OnWriteScore(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	ASSERT(wDataSize==sizeof(DBR_GR_SendWriterScroe));
	if (wDataSize!=sizeof(DBR_GR_SendWriterScroe)) return false;
	...
	case SUB_GR_USER_LOOKON_REQ:	//旁观桌子
		{
			...
			if (wSubCmdID==SUB_GR_USER_SIT_REQ)	
			{
				...
				{
					wSearchID=pUserSitReq->wChairID;
				}

				//坐下位置
				if (wSearchID==INVALID_CHAIR)
				{
					//构造数据
					CMD_GR_SitFailed SitFailed;
					ZeroMemory(&SitFailed,sizeof(SitFailed));
					_snprintf(SitFailed.szFailedDescribe,sizeof(SitFailed.szFailedDescribe),TEXT("这游戏桌已经人满为患了，请选择其他桌子！"));
				...
				{
					pTableFrame->PerformSitDownAction(wSearchID,pIServerUserItem,pUserSitReq->szTablePass);
				}
			}
			else
			{
	...
	case SUB_GR_CALLUSER:			//呼叫用户
		{
			...
			//转发消息
// 			m_pITCPNetworkEngine->SendDataBatch(MDM_GR_USER,SUB_GR_CALLUSER,pUserChat,wDataSize);
// 			m_AndroidUserManager.SendDataToClient(MDM_GR_USER,SUB_GR_CALLUSER,pUserChat,wDataSize);
			//转发消息
			//SendData(pIServerUserItemSend,MDM_GR_USER,SUB_GR_USER_WISPER,pWisper,wDataSize);
			SendData(pIServerUserItemRecv,MDM_GR_USER,SUB_GR_CALLUSER,pUserChat,wDataSize);
	...
	case SUB_GR_LOOK_USER_IP:	//查看地址
		{
			...
			_snprintf(szMessage,CountArray(szMessage),TEXT("[ %s ]：%d.%d.%d.%d\n金币:%ld\n银行:%ld"),pIServerUserItemRecv->GetAccounts(),
				pcbUserIP[0],pcbUserIP[1],pcbUserIP[2],pcbUserIP[3], pIServerUserItemRecv->GetUserScore()->lScore, pIServerUserItemRecv->GetUserScore()->lInsureScore);
...
bool CAttemperEngineSink::OnSocketPresent(WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	if (m_pGameServiceOption->wServerType==GAME_GENRE_EDUCATE)
	{
		SendProMessage(pIServerUserItem,TEXT("练习房间不可以使用此功能！"),SMT_INFO|SMT_EJECT);
		return true;
	}
...
bool CAttemperEngineSink::OnSocketBank(WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	case SUB_GF_BANK_TRANSFER:				//转出事件
		{
			return OnEventBankTransfer(pData,wDataSize,dwSocketID);
		}
	case SUB_GF_BANK_SECURITY:              //全安事件
		{
			return OnEventBankSecurity(pData,wDataSize,dwSocketID);
		}
	}
...
bool CAttemperEngineSink::SendUserItem(IServerUserItem * pIServerUserItem, DWORD dwSocketID)
{
	...
	//pUserInfoHead->cbAndroid = pUserData->cbAndroid;			//add 0506ygs 机器人标识
...
bool CAttemperEngineSink::OnEventFlower(const void * pData, WORD wDataSize, IServerUserItem * pSendIServerUserItem)
{
	...
	if ( pIRcvUserItem != NULL )
	{
		...
		LONG lFlowerPrice = LONG(pFlowerInfo->lPrice * pGift->wFlowerCount * ((pSendUserData->cbMemberOrder > 0 ? pFlowerInfo->cbDiscount : 100) / 100.));

		//获取欢乐豆
		LONGLONG lInsureScore = GetBankStorageGold(pSendIServerUserItem);

		if (lFlowerPrice<=0) return true;
		...
		{
			//发送消息
			SendProMessage(pSendIServerUserItem,TEXT("购买失败，您购物欢乐豆不够！"),SMT_EJECT|SMT_INFO);
			return true;
		}
...
bool CAttemperEngineSink::OnEventProperty(const void * pData, WORD wDataSize, IServerUserItem * pIServerUserItem)
{
	...
	for ( int nIndex = 0; nIndex < CountArray(dwPropertyCount); ++ nIndex )
	{
		...
		{
			lPrice = LONG(lPropertyPrice[nIndex] * (pPropertyHeadInfo->dwPachurseCount * nDiscount / 100.));
			break;
		}
	...
	//获取欢乐豆
	LONGLONG lInsureScore = GetBankStorageGold(pIServerUserItem);

	if (lPrice<=0) return true;
	...
	{
		//发送消息
		SendProMessage(pIServerUserItem,TEXT("购买失败，您购物欢乐豆不够！"),SMT_EJECT|SMT_INFO);		
		return true;
	}
	...
	{
		LONGLONG lNowScore = pTargetUserData->UserScoreInfo.lScore;
		if ( lNowScore < 0)
		{
			//变量定义
	...
	{
		TCHAR szMessage[128];
		_sntprintf(szMessage,CountArray(szMessage),TEXT("[ %s ]赠送[ %s ](%s)"), pConsumeUserData->szAccounts, pTargetUserData->szAccounts, 
			g_PropertyDescribe[nPropertyID].szName);
		SendProMessage(INVALID_WORD, szMessage, SMT_INFO);

		_sntprintf(szMessage,CountArray(szMessage),TEXT("[ %s ]赠送您(%s)"), pConsumeUserData->szAccounts, g_PropertyDescribe[nPropertyID].szName);
		SendProMessage(pTargetUserItem, szMessage, SMT_INFO);
	}
...
bool CAttemperEngineSink::OnEventExchangeCharm(const void * pData, WORD wDataSize, IServerUserItem * pIServerUserItem)
{
	...
	/////////////////////////////////////////////////////////////////////////////
	//系统消息
	TCHAR szMessage[] = TEXT("对不起，魅力兑换暂未开放！");
	...
	//系统消息
	TCHAR szMessage2[] = TEXT("恭喜您，兑换成功");
	SendProMessage(pIServerUserItem, szMessage2, SMT_EJECT|SMT_INFO);
...
bool CAttemperEngineSink::OnEventBankDrawoutGold(const void * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	//判断状态
	if(m_pGameServiceOption->wServerType==GAME_GENRE_GOLD /*&& NULL==m_pTableFrame[0].m_pITableUserActionEX*/)
	{
...
//			return true;
	...
	{
		SendRoomMessage(dwSocketID,TEXT("非欢乐豆游戏不能转账，不能进行银行操作！"),SMT_EJECT);
		return true;
	}

	//类型转换
	CMD_GF_BankGet *pBankGet= (CMD_GF_BankGet*)pData;
	LONGLONG lStorageGold = pServerUserData->UserScoreInfo.lInsureScore;
	...
	//密码效验
	if (lstrcmp(pServerUserData->szBankPassword,pBankGet->szPassword)!=0)
	{
		if(pBankGet->cbGameAction)
	...
	//回调通知
	if(pServerUserData->wTableID!=INVALID_TABLE && NULL!=m_pTableFrame[0].m_pITableUserActionEX)
	{
		CTableFrame *pTableFrame=m_pTableFrame+pServerUserData->wTableID;
		if (NULL!=pTableFrame->m_pITableUserActionEX) 
			pTableFrame->m_pITableUserActionEX->OnActionUserBank(pIServerUserItem->GetChairID(),pIServerUserItem);
	}
...
bool CAttemperEngineSink::OnEventBankStorage(const void * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	LONGLONG lGameGold = pServerUserData->UserScoreInfo.lGameGold;
	...
	if(m_pGameServiceOption->wServerType==GAME_GENRE_GOLD)
	{
		if(pServerUserData->wTableID!=INVALID_TABLE/* && NULL==m_pTableFrame[0].m_pITableUserActionEX*/)
		{
			if (pBankStorage->cbGameAction==0)
				SendRoomMessage(dwSocketID,TEXT("请退出游戏后，再进行存储操作!"),SMT_EJECT);
			else
				SendGameMessage(pIServerUserItem,TEXT("请退出游戏后，再进行存储操作!"),SMT_EJECT);
			return true;
		}
	}
	else
	{
		SendRoomMessage(dwSocketID,TEXT("非欢乐豆游戏不能转账，不能进行银行转账操作！"),SMT_EJECT);
		return true;
	}

	//判断状态
	if (NULL!=m_pTableFrame[0].m_pITableUserActionEX && US_PLAY==pServerUserData->cbUserStatus)
	{
		if (pBankStorage->cbGameAction) 
			SendGameMessage(dwSocketID,TEXT("请在游戏结束后，再进行存储操作！"),SMT_EJECT);
		else
			SendRoomMessage(dwSocketID,TEXT("请在游戏结束后，再进行存储操作！"),SMT_EJECT);
		return true;
	}
	...
	//密码效验
// 	if (!pBankStorage->cbGameAction && lstrcmp(pServerUserData->szBankPassword,pBankStorage->szPassword)!=0)
// 	{
// 		if(pBankStorage->cbGameAction)
// 			SendGameMessage(dwSocketID,TEXT("密码有误，请查证后再次尝试操作！"),SMT_EJECT);
// 		else
...
bool CAttemperEngineSink::OnEventBankTransfer(const void * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	ASSERT(wDataSize==sizeof(CMD_GF_BankTransfer));
	if (wDataSize!=sizeof(CMD_GF_BankTransfer)) return false;
	...
	CMD_GF_BankTransfer *pBankTransfer=(CMD_GF_BankTransfer *)pData;
	LONGLONG lBankGold = pServerUserData->UserScoreInfo.lInsureScore;
	LONGLONG lGameGold = pServerUserData->UserScoreInfo.lGameGold;
	...
	//	if(pServerUserData->wTableID!=INVALID_TABLE && NULL==m_pTableFrame[0].m_pITableUserActionEX)
	//	{
	//		SendBankResult(pIServerUserItem,3,0,TEXT("请退出游戏后，再进行存储操作!"),pBankTransfer->cbGameAction);
	//		return true;
	//	}
	}	
	else
	...
	//判断状态
	if (US_PLAY==pServerUserData->cbUserStatus)
	{
		//SendBankResult(pIServerUserItem,3,0,TEXT("您正在游戏状态中，不能进行银行转账操作！"),pBankTransfer->cbGameAction);
		//SendRoomMessage(dwSocketID,TEXT("您正在游戏状态中，不能进行银行转账操作！"),SMT_EJECT);
		//return true;
	}


	//数据效验
	if (pBankTransfer->dwTagGameID==pServerUserData->dwGameID)
	{
		//SendBankResult(pIServerUserItem,3,0,TEXT("银行转帐非法操作，目标ID不能是自已!"),pBankTransfer->cbGameAction);
		SendRoomMessage(dwSocketID,TEXT("银行转帐非法操作，目标ID不能是自已!"),SMT_EJECT);
		return false;
	}

	//效验数据
	ASSERT(pBankTransfer->lTransferValue>0 && pBankTransfer->lTransferValue<=lBankGold);
	if (pBankTransfer->lTransferValue<0 || pBankTransfer->lTransferValue>lBankGold) return false;

	////密码效验
	//if (lstrcmp(pServerUserData->szBankPassword,pBankTransfer->szPassword)!=0)
	//{
	//	//SendBankResult(pIServerUserItem,3,0,TEXT("密码有误，请查证后再次尝试操作！"),pBankTransfer->cbGameAction);
	//	SendRoomMessage(dwSocketID,TEXT("密码有误，请查证后再次尝试操作！"),SMT_EJECT);
	//	return true;
	//}

	//密码效验
	if (lstrcmp(pIServerUserItem->GetPassword(),pBankTransfer->szPassword)!=0)
	{
		SendRoomMessage(dwSocketID,TEXT("密码有误，请查证后再次尝试操作！"),SMT_EJECT);
		return true;
	}
	...
	//银行状态
	//if (pIServerUserItem->IsLockedUserBank())
	//{
	//	//SendBankResult(pIServerUserItem,3,0,TEXT("您上一次的银操请求尚未完成，请稍后！"),pBankTransfer->cbGameAction);
	//	SendRoomMessage(dwSocketID,TEXT("您上一次的银操请求尚未完成，请稍后！"),SMT_EJECT);
	//	return true;
	//}


	//计算扣税
	LONGLONG lRevenue=pBankTransfer->lTransferValue * 0.02;
	LONGLONG lSwapeScore = pBankTransfer->lTransferValue-lRevenue;

	//枚举用户
	IServerUserItem * pITargetUserItem=NULL;
	IServerUserItem * pEnumUserItem=NULL;
	INT_PTR nOnlineCount=m_ServerUserManager.GetOnLineCount();
	for (INT_PTR i = 0; i < nOnlineCount; i++)
	{
		pEnumUserItem=m_ServerUserManager.EnumOnLineUser((WORD)i);
		if (pEnumUserItem->GetUserData()->dwGameID==pBankTransfer->dwTagGameID)
		{
			pITargetUserItem=pEnumUserItem;
			break;
		}
	}
	...
	{
		//SendRoomMessage(dwSocketID,TEXT("对不起，用户不在房间，不能转账！"),SMT_EJECT);
		bIsOnline = false;
		//return true;
	}
	...
	if(!bIsOnline)
	{
		...
		////修改帐款
		ModifyBankStorageGold(pIServerUserItem,-pBankTransfer->lTransferValue);
		//发送积分
		SendUserScore(pIServerUserItem);
	}
	else
	{
		tagServerUserData *pTargetUserData = pITargetUserItem->GetUserData();
		////修改帐款
		ModifyBankStorageGold(pIServerUserItem,-pBankTransfer->lTransferValue);
		ModifyBankStorageGold(pITargetUserItem,lSwapeScore);
		//发送积分
		SendUserScore(pIServerUserItem);
		SendUserScore(pITargetUserItem);

		TCHAR szTemp[256];
		_snprintf(szTemp, 256, TEXT("您已成功给[ %s ]转入[ %I64d ]游戏币！(扣税 %I64d)"), pTargetUserData->szAccounts, pBankTransfer->lTransferValue, lRevenue);
		SendRoomMessage(pIServerUserItem, szTemp, SMT_EJECT);

		_snprintf(szTemp, 256, TEXT("您接收到[ %s ]转入的[ %I64d ]游戏币，请注意查收！(已扣税 %I64d)"), pServerUserData->szAccounts, lSwapeScore, lRevenue);
		SendRoomMessage(pITargetUserItem, szTemp, SMT_EJECT);

	}
	DBR_GR_BankTransfer BankTransfer;
	ZeroMemory(&BankTransfer,sizeof(BankTransfer));

	BankTransfer.dwUserID=pServerUserData->dwUserID;
	BankTransfer.dwTagGameID=pBankTransfer->dwTagGameID;
	BankTransfer.lTransferCount=lSwapeScore;
	BankTransfer.lRevenue=lRevenue;
	BankTransfer.lCurrentScore=lGameGold;
	BankTransfer.lCurrentBankScore=lBankGold;
	BankTransfer.dwClientIP=pIServerUserItem->GetClientIP();
	BankTransfer.cbGameAction=pBankTransfer->cbGameAction;
	BankTransfer.cbUserOneline=bIsOnline;

	//投递请求
	m_pIDataBaseEngine->PostDataBaseRequest(DBR_GR_BANK_TRANSFER,0,&BankTransfer,sizeof(BankTransfer));

	////回调通知
	//if(pServerUserData->wTableID!=INVALID_TABLE && NULL!=m_pTableFrame[0].m_pITableUserAction)
	//{
	//	CTableFrame *pTableFrame=m_pTableFrame+pServerUserData->wTableID;
	//	if (NULL!=pTableFrame->m_pITableUserAction) 
	//		pTableFrame->m_pITableUserAction->OnActionUserBank(pIServerUserItem->GetChairID(),pIServerUserItem);
	//}

	return true;
}
//安全事件
bool CAttemperEngineSink::OnEventBankSecurity(const void * pData, WORD wDataSize, DWORD dwSocketID)
{

	ASSERT(wDataSize==sizeof(CMD_GF_BankSecurity));
	if (wDataSize!=sizeof(CMD_GF_BankSecurity)) return false;
	...
	CMD_GF_BankSecurity *pBankSecurity=(CMD_GF_BankSecurity *)pData;


	//密码效验
	//if (lstrcmp(pServerUserData->szBankPassword,pBankSecurity->szPassword)!=0)
	if (lstrcmp(pIServerUserItem->GetPassword(),pBankSecurity->szPassword)!=0)
	{
		//SendBankResult(pIServerUserItem,4,0,TEXT("您的银行密码不正确，请查证！"),pBankSecurity->cbGameAction);
		SendRoomMessage(dwSocketID,TEXT("您的银行密码不正确，请查证！"),SMT_EJECT);
		return true;
	}
	//else SendBankResult(pIServerUserItem,4,0,TEXT("银行密码修改成功！"),pBankSecurity->cbGameAction);
	SendRoomMessage(dwSocketID,TEXT("银行密码修改成功！"),SMT_EJECT);


	//更新密码
	CopyMemory(pServerUserData->szBankPassword,pBankSecurity->szNewPassword,sizeof(pServerUserData->szBankPassword));

	DBR_GR_BankSecurity BankSecurity;
	ZeroMemory(&BankSecurity,sizeof(BankSecurity));
	BankSecurity.dwUserID=pIServerUserItem->GetUserID();
	BankSecurity.dwClientIP=pIServerUserItem->GetClientIP();
	CopyMemory(BankSecurity.szNewPassword,pBankSecurity->szNewPassword,sizeof(BankSecurity.szNewPassword));

	//投递请求
	m_pIDataBaseEngine->PostDataBaseRequest(DBR_GR_BANK_SECURITY,0,&BankSecurity, sizeof(BankSecurity));
	...
	case PROP_NEGAGIVE :			//负分清零
		{
			...
			if( 0 <= lNowScore)
			{
				...
				if ( pIServerUserItem == pTargetUserItem ) 
					...
					_sntprintf(szMessage,CountArray(szMessage),TEXT("[ %s ]现在的积分已经是非负数，不需要使用负分清零道具！"), pTargetUserData->szAccounts);
	...
	case PROP_FLEE :				//逃跑清零
		{
			...
			if ( lNowFleeCount == 0 )
			{
				...
				if ( pIServerUserItem == pTargetUserItem ) 
					...
					_sntprintf(szMessage,CountArray(szMessage),TEXT("[ %s ]现在的逃跑率已经为0，不需要使用逃跑清零道具！"), pTargetUserData->szAccounts);
...
void CAttemperEngineSink::ModifyGameGold(IServerUserItem * pIServerUserItem,LONGLONG lGameGold)
{
	...
	if(m_pGameServiceOption->wServerType == GAME_GENRE_GOLD)
	{
		...
		{
			LONGLONG lUserScore=pServerUserData->UserScoreInfo.lScore;
		...
		if((pIServerUserItem->GetTableID()!=INVALID_TABLE) && (lLessScore>0) && (lLessScore>pServerUserData->UserScoreInfo.lScore))
		{
			...
			if (!(pTableFrame->IsUserPlaying(pIServerUserItem)))
			{
				//发送信息
				TCHAR szDescribe[256]=TEXT("");
				_snprintf(szDescribe,sizeof(szDescribe),TEXT("加入游戏至少需要 %I64d 的欢乐豆，您的欢乐豆不够，不能加入！"),lLessScore);
		...
		if((pIServerUserItem->GetTableID()!=INVALID_TABLE) && (lMaxScore>m_pGameServiceOption->lLessScore) && (lMaxScore<pServerUserData->UserScoreInfo.lScore))
		{
			...
			if (!(pTableFrame->IsUserPlaying(pIServerUserItem)))
			{
				//发送信息
				TCHAR szDescribe[256]=TEXT("");
				_snprintf(szDescribe,sizeof(szDescribe),TEXT("您的欢乐豆超过本房间的最大限制额度%I64d,请更换游戏房间!"),lMaxScore);
				SendRoomMessage(pIServerUserItem,szDescribe,SMT_INFO);
...
bool CAttemperEngineSink::OnBankResult( DWORD dwContextID, VOID * pData, WORD wDataSize )
{
	ASSERT(wDataSize==sizeof(DBR_GR_BankResult));
	if (wDataSize!=sizeof(DBR_GR_BankResult)) return false;

	DBR_GR_BankResult * pBankResult=(DBR_GR_BankResult*)pData;

	//在线转账不处理
	if(pBankResult->cbUserOnline == TRUE)
		return true;


	IServerUserItem* pSendUserItem = m_ServerUserManager.SearchOnLineUser(pBankResult->dwUserID);
	...
	//转账成功
	if(pBankResult->lErrorCode == 0)
	{
		_snprintf(szMsg, 256, "您已成功给ID为:%d 转入[ %I64d ] 欢乐豆(税收 %I64d)!", pBankResult->dwTagGameID, pBankResult->lModifyGold, pBankResult->lRevenue);
		SendRoomMessage(pSendUserItem, szMsg, SMT_EJECT);
	}
	else
	{
		//返还积分
		ModifyBankStorageGold(pSendUserItem,pBankResult->lModifyGold+pBankResult->lRevenue);
		//积分通知
		SendUserScore(pSendUserItem);

		_snprintf(szMsg, 256, "您查找的用户不存在,转账失败！");
		SendRoomMessage(pSendUserItem, szMsg, SMT_EJECT);
	}
	return true;
