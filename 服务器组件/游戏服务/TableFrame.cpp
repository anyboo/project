...
CTableFrame::CTableFrame()
{
	...
	ZeroMemory(m_OriginScore,sizeof(m_OriginScore));
...
bool __cdecl CTableFrame::PerformSitDownAction(WORD wChairID, IServerUserItem * pIServerUserItem, LPCTSTR szPassword)
{
	...
	//////////////////////////////////////////////////////////////////////////
	//MODIF 4人椅子处理啊。。
	if(m_pGameServiceAttrib->wKindID == 5 && m_pGameServiceAttrib->wChairCount == 4 )
	{
		if(wChairID==3 || wChairID == 1)
		{
			SendSitFailedPacket(pIServerUserItem,TEXT("抱歉，此椅子现在不允许玩家进入！"));
			return false;
		}
	}
	//////////////////////////////////////////////////////////////////////////
	...
	else
	{
		...
		if (lLessScore!=0L || lMaxScore>lLessScore)
		{
			if (pUserData->UserScoreInfo.lScore<lLessScore)
			{
				...
				{
					_snprintf(szDescribe,sizeof(szDescribe),TEXT("加入游戏至少需要 %I64d 个欢乐豆，您的欢乐豆不够，不能加入！"),lLessScore);
				}
				else
				{
					_snprintf(szDescribe,sizeof(szDescribe),TEXT("加入游戏至少需要 %I64d 个游戏积分，您的积分不够，不能加入！"),lLessScore);
				}
				SendSitFailedPacket(pIServerUserItem,szDescribe);
			...
			if (lMaxScore>lLessScore && lMaxScore<pUserData->UserScoreInfo.lScore)
			{
				...
				{
					_snprintf(szDescribe,sizeof(szDescribe),TEXT("您的游戏积分超过本房间的最大限制额度%I64d,请更换游戏房间!"),lMaxScore);
				}
				SendSitFailedPacket(pIServerUserItem,szDescribe);
...
bool __cdecl CTableFrame::ConcludeGame()
{
	...
	if (GAME_GENRE_GOLD==m_pGameServiceOption->wServerType)
	{
		...
		for (WORD wIndex=0; wIndex<CountArray(m_ScoreInfo); ++wIndex) 
		{
			...
			if ((m_OriginScore[wIndex]+m_ScoreInfo[wIndex].lScore)<0L)
			{
				TCHAR szError[128];
				_sntprintf(szError,CountArray(szError),TEXT("KindName:%s,ServerName:%s,OriginScore:%I64d,ScoreInfo:%I64d"),
					m_pGameServiceAttrib->szKindName,m_pGameServiceOption->szGameRoomName,m_OriginScore[wIndex],m_ScoreInfo[wIndex].lScore);
		...
		for (WORD i=0;i<m_wChairCount;i++)
		{
			if (m_pIUserItem[i]!=NULL)
			{
				...
				if (pUserData->UserScoreInfo.lScore<lLessScore)
				{
					...
					if (m_pGameServiceOption->wServerType==GAME_GENRE_GOLD)
					{
						_snprintf(szDescribe,sizeof(szDescribe),TEXT("【 %s 】的欢乐豆少于 %I64d，无法继续游戏！"),pUserData->szAccounts, lLessScore);
					}
					else 
					{
						_snprintf(szDescribe,sizeof(szDescribe),TEXT("【 %s 】的游戏积分少于 %I64d，无法继续游戏！"),pUserData->szAccounts, lLessScore);
					}
					//SendGameMessage(pIServerUserItem,szDescribe,SMT_EJECT|SMT_INFO|SMT_CLOSE_GAME);
					//发送消息-金币不足提示
					if(m_wChairCount < MAX_CHAIR_NORMAL)
					{
						for (WORD j=0;j<m_wChairCount;j++)
						{
							if ((m_pIUserItem[j]!=NULL))
							{
								if(pIServerUserItem != m_pIUserItem[j])
									SendGameMessage(m_pIUserItem[j],szDescribe,SMT_INFO|SMT_EJECT);
								else
									SendGameMessage(pIServerUserItem,szDescribe,SMT_EJECT|SMT_INFO|SMT_CLOSE_GAME);
							}
						}
...
bool __cdecl CTableFrame::WriteUserScore(IServerUserItem * pIServerUserItem, LONGLONG lScore, LONGLONG lRevenue, enScoreKind ScoreKind, LONG lPlayTimeCount)
{
	...
	//设置变量
	if (pIServerUserItem->GetChairID()<m_pGameServiceAttrib->wChairCount)
	{
	...
	if ( m_pGameServiceOption->wServerType != GAME_GENRE_GOLD && m_pGameServiceOption->wServerType != GAME_GENRE_MATCH )
	{
		...
		if ( 0 < lScore )
		{
			...
			if ( 0 < lUseableTime && lPassedTime <= lUseableTime ) 
			{
				...
				{
					lScore *= 2;
					_sntprintf(szMessage,CountArray(szMessage),TEXT("[ %s ] 使用了[ 双倍积分卡 ]，得分翻倍！)"), 
						pServerUserData->szAccounts);
				}
				else if ( cbPropertyType == PROP_FOURDOLD )
				{
					lScore *= 4;
					_sntprintf(szMessage,CountArray(szMessage),TEXT("[ %s ] 使用了[ 四倍积分卡 ]，得分翻四倍！)"), 
						pServerUserData->szAccounts);
				}
			}
		}
		else if ( lScore < 0 )
		{
			...
			{
				lScore = 0;
				_sntprintf(szMessage,CountArray(szMessage),TEXT("[ %s ] 使用了[ 护身符卡 ]，积分不变！"), pServerUserData->szAccounts);
			}
		}
		...
		if (szMessage[0]!=0)
		{
			...
			for (INT_PTR i=0;i<m_LookonUserItemPtr.GetCount();i++)
			{
				if (IsClientReady(m_LookonUserItemPtr[i])==true)
				{
					SendGameMessage(m_LookonUserItemPtr[i], szMessage, SMT_INFO);
				}
			}
		}
	}
...
bool __cdecl CTableFrame::StartGame()
{
	...
	for (WORD i=0;i<m_wChairCount;i++)
	{
		if (m_pIUserItem[i]!=NULL)
		{
			...
			m_OriginScore[i]=m_pIUserItem[i]->GetUserScore()->lScore;
		}
	}
...
WORD CTableFrame::GetNullChairID()
{
	...
	else
	{
		for (WORD i=0;i<m_wChairCount;i++)
		{
			//////////////////////////////////////////////////////////////////////////
			//MODIF 
			if (m_pGameServiceAttrib->wKindID == 5 && m_wChairCount == 4)
			{
				if(i==1||i==3) continue;
				if(rand()%2==0) 
				{
					if(m_pIUserItem[2]==NULL) return 2;
				}
			}
			//////////////////////////////////////////////////////////////////////////
			if (m_pIUserItem[i]==NULL)
