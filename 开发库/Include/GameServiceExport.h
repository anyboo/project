...
#define IDI_DISTRIBUTE_LIKAI			(IDI_ATTEMPER_START+7)				//分配用户
//时间定义
...
#define TIME_LIIBUTE_ANDROID			15L									//分配用户
//用户时间
...
public:
	//获取税收
	virtual LONGLONG __cdecl GetUserRevenue()=NULL;
	//游戏时间
...
public:
	...
	//修改欢乐豆
	virtual bool __cdecl ModifyBankStorageGold(LONGLONG lStorageCount)=NULL;
	//积分信息
	virtual tagUserScore* __cdecl GetUserScoreModifyInfo()=NULL;
	////锁住银行
	//virtual void __cdecl LockUserBank()=NULL;
	////解锁银行
	//virtual void __cdecl UnlockUserBank()=NULL;
	////加锁状态
	//virtual bool __cdecl IsLockedUserBank()=NULL;
...
public:
	//写入积分
	virtual bool __cdecl WriteUserScore(WORD wChairID, LONGLONG lScore, LONGLONG lRevenue, enScoreKind ScoreKind, LONG lPlayTimeCount=-1)=NULL;
	//写入积分
	virtual bool __cdecl WriteUserScore(IServerUserItem * pIServerUserItem, LONGLONG lScore, LONGLONG lRevenue, enScoreKind ScoreKind, LONG lPlayTimeCount=-1)=NULL;
};
...
public:
	...
	//发送强退
	virtual bool __cdecl SendMainUser(WORD wSubCmdId, VOID* pData, WORD wDataSize)=NULL;
};
