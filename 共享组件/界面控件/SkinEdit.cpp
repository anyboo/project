...
void CMyComboBox::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	rect -= rect.TopLeft();
 	...
 	int iWidth=CSkinEdit::m_SkinAttribute.m_ImageBottom.GetWidth()/5;*/
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	if (point.x>=0/*rect.right-iWidth-1*/)
	{
		if (m_bFocus!=0x02)
		{
			m_bFocus=0x02;
...
void CMyComboBox::OnPaint()
{
	...
	//获取位置
	CImageHandle ImageHandleBottom(&CSkinEdit::m_SkinAttribute.m_ImageBottom);
	int iWidth=CSkinEdit::m_SkinAttribute.m_ImageBottom.GetWidth()/5,iHeight=CSkinEdit::m_SkinAttribute.m_ImageBottom.GetHeight();
...
//}
	...
	GetWindowRect(&rect); //获得窗口矩形
	rect -= rect.TopLeft();
	CPen LinePen(PS_SOLID,1,RGB(92,100,105));
	CPen LinePen1(PS_SOLID,2,RGB(255,255,255));
 	//rect.right-=1;
 	//rect.bottom-=1;
	//pDC->SelectObject(&LinePen);
	//pDC->Rectangle( rect.right - 6,rect.bottom - 6,rect.right,rect.bottom );
...
// 	//画边框
	CPen psBack(PS_SOLID,1, RGB(92,100,105));
	pDC->SelectObject(&psBack);
	pDC->Rectangle( rect.left,rect.top,rect.right, rect.bottom);

// 	pDC->SelectObject(&LinePen1);
// 
// 	pDC->RoundRect(rect.left,rect.top,rect.right, rect.bottom, 9,9);
// 
// 	pDC->SelectObject(&LinePen);
// 	pDC->RoundRect(rect.left+1,rect.top+1,rect.right-1, rect.bottom-1,2,2);
	...
	if (m_bFocus&0x01)
	{
		CSkinEdit::m_SkinAttribute.m_ImageBottom.AlphaDrawImage(pDC,rect.right-iWidth-3,3,iWidth,iHeight,iWidth*2,0,RGB(22,0,0));
	}
	else if(m_bFocus&0x02)
	{
		CSkinEdit::m_SkinAttribute.m_ImageBottom.AlphaDrawImage(pDC,rect.right-iWidth-3,3,iWidth,iHeight,iWidth*1,0,RGB(22,0,0));
	}
	else
	{	
		//AfxMessageBox(1);
		CSkinEdit::m_SkinAttribute.m_ImageBottom.AlphaDrawImage(pDC,rect.right-iWidth-3,3,iWidth,iHeight,iWidth*3,0,RGB(22,0,0));
...
VOID CMyComboBox::PreSubclassWindow()
{
	...
	if (GetComboBoxInfo(&ComboBoxInfo)==TRUE)
	{
		...
		{
			//ComboBoxInfo.hwndItem.MoveWindow(0,0,0,0);
			CWnd* pWnd = CWnd::FromHandle(ComboBoxInfo.hwndItem);
			//pWnd->SetWindowPos(NULL, 3,5,rect.Width()-iWidth-5,rect.Height(), 0);
			pWnd->SetWindowPos(NULL,5,5,rcClient.Width()-29,rcClient.Height()-5,SWP_NOZORDER);
		}
	}
