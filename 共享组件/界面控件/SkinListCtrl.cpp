...
//默认属性
#define LIST_STYTE					LVS_EX_FULLROWSELECT|LVS_EX_HEADERDRAGDROP|LVS_EX_ONECLICKACTIVATE|LVS_EX_UNDERLINEHOT
...
bool CSkinListAttribute::UpdateSkinResource(ISkinResource * pISkinResource)
{
	...
	m_dwHeadHight = SkinListCtrlResource.dwHeadHight;
	m_ImageHeadLeft.SetLoadInfo(SkinListCtrlResource.LoadInfoImageBackLeft);
	m_ImageHeadMid.SetLoadInfo(SkinListCtrlResource.LoadInfoImageBackMid);
	m_ImageHeadRight.SetLoadInfo(SkinListCtrlResource.LoadInfoImageBackRight);
	return true;
}
...
// 		m_BufferDC.Draw3dRect(ClientRect.left,ClientRect.top,ClientRect.Width(),ClientRect.Height(),RGB(250,250,250),RGB(250,250,250));
 		Default();
// 	}

// 	CImageHandle ImageHandle(&m_SkinAttribute.m_ImageHead);
// 	CRect rcHead = rcItemRect;
// 	CSkinImage*		ImageHead = (&m_SkinAttribute.m_ImageHead);
// 
// 	ImageHead->BitBlt(m_BufferDC,rcHead.left,rcHead.top);
// 
// 	for (INT nXPos=rcHead.left+ImageHead->GetWidth();nXPos<rcHead.right;nXPos+=ImageHead->GetWidth())
// 	{
// 		ImageHead->BitBlt(m_BufferDC,nXPos,rcHead.top);
// 	}
...
LRESULT CSkinHeaderCtrl::OnLayout( WPARAM wParam, LPARAM lParam )
{
	...
	pwpos->cy = m_SkinAttribute.m_dwHeadHight;
	prc->top = m_SkinAttribute.m_dwHeadHight;
	return lResult;
}
//绘画函数
void CSkinHeaderCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	CImageHandle ImageHandle1(&m_SkinAttribute.m_ImageHeadLeft);
	CImageHandle ImageHandle2(&m_SkinAttribute.m_ImageHeadMid);
	CImageHandle ImageHandle3(&m_SkinAttribute.m_ImageHeadRight);
	CRect rcHead = rcItemRect;
	CSkinImage*	ImageHeadLeft =(&m_SkinAttribute.m_ImageHeadLeft);
	CSkinImage*	ImageHeadMid =(&m_SkinAttribute.m_ImageHeadMid);
	CSkinImage*	ImageHeadRight =(&m_SkinAttribute.m_ImageHeadRight);

	ImageHeadLeft->BitBlt(m_BufferDC,rcHead.left,rcHead.top);

	for (INT nXPos=rcHead.left+ImageHeadLeft->GetWidth();nXPos<rcHead.right-ImageHeadRight->GetWidth();nXPos+=ImageHeadMid->GetWidth())
	{
		ImageHeadMid->BitBlt(m_BufferDC,nXPos,rcHead.top);
	}

	ImageHeadRight->BitBlt(m_BufferDC,rcHead.right-ImageHeadRight->GetWidth(),rcHead.top);
...
//	else m_SkinAttribute.m_ImageHead.StretchBlt(pDC->m_hDC,rcItemRect.left,rcItemRect.top,rcItemRect.Width(),rcItemRect.Height()+10);
// 	CRect rcItem=rcItemRect;
// 	rcItem.left=rcItem.right-1;
// 	rcItem.top+=5;
// 	rcItem.bottom-=5;
//  	if (bSelected) pDC->Draw3dRect(&rcItemRect,RGB(125,125,125),RGB(125,125,125));
//  	else pDC->Draw3dRect(&rcItemRect,RGB(125,125,125),RGB(125,125,125));
// 
// 	if (bSelected) pDC->Draw3dRect(&rcItem,RGB(125,125,125),RGB(125,125,125));
// 	else pDC->Draw3dRect(&rcItem,RGB(125,125,125),RGB(125,125,125));
// 
// 	pDC->Draw3dRect(&rcItemRect,RGB(91,91,91),RGB(91,91,91));
