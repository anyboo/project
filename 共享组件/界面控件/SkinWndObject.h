...
public:
    ...
    CSkinImage						m_FramLeft;						//标题图片
	CSkinImage						m_BottomM;							//标题图片
	CSkinImage						m_FramRight;						//标题图片
	CSkinImage						m_BottomL;						//标题图片
	CSkinImage						m_BottomR;							//标题图片
	CSkinImage						m_BackGroundTop;					//背景上部
	CSkinImage						m_BackGroundMid;					//背景中部
	CSkinImage						m_BackGroundBtm;					//背景尾部
	//函数定义
protected:
	//构造函数
	CSkinWndAttribute();
...
class CSkinWndAttribute1 : public CSkinResourceManager
{
	friend class CSkinWndObject1;
...
protected:
	...
	bool OnMouseMoveEvent1(CPoint ClientPoint);
	//鼠标左键事件
	bool OnLMouseDownEvent(CPoint ClientPoint);
	bool OnLMouseDownEvent1(CPoint ClientPoint);
	//鼠标左键事件
	bool OnLMouseUpEvent(CPoint ClientPoint);
	bool OnLMouseUpEvent1(CPoint ClientPoint);
	//鼠标左键事件
...
//窗口类控制
class SKIN_CONTROL_CLASS CSkinWndObject1
{
	//状态变量
protected:
	bool							m_bActive;							//是否激活
...
private:
	...
	//静态变量
public:
	static CSkinWndAttribute1		m_SkinAttribute;					//窗口属性

	//函数定义
public:
	//构造函数
	CSkinWndObject1(CWnd * pWndSkin);
	//析构函数
	virtual ~CSkinWndObject1(void);
...
public:
	...
	int GetYExcursionPos() { return m_nYExcursionPos; }
	//获取属性接口
	static CSkinWndAttribute1 * GetSkinAttribute() { return &m_SkinAttribute; };
