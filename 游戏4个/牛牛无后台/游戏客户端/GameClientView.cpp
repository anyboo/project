...
// 	m_btBegin.LoadStdImage(L"OX\\begin.png");
// 	m_btCall.LoadStdImage(L"OX\\call.png");
// 	m_btCalling.LoadStdImage(L"OX\\calling.png", false);
	m_pngWaitBet.LoadImage(hInstance,"IDB_WAITBET");
	m_pngWaitCall.LoadImage(hInstance,"IDB_WAITCALL");
	m_pngBanker.LoadImage(hInstance,"IDB_BANKER2");
	m_pngNiuNum.LoadImage(hInstance,"IDB_NIUNUM");
	ImageTimeBack.LoadImage(hInstance,TEXT("TIME_BACK"));
	ImageTimeNumber.LoadImage(hInstance,TEXT("TIME_NUMBER"));
	return;
}
...
{
	if(GetUserInfo(m_wMeChairID) == NULL)
	{
		return ;
	}
	//如果有此权限
...
LPCTSTR CGameClientView::NumToString( LONGLONG lLeaveScore)
{
	CString static strUserScore=TEXT(""),strTmpNumber1,strTmpNumber2;
	strTmpNumber1.Empty();
	strTmpNumber2.Empty();
	strUserScore.Empty();
	if(lLeaveScore==0) strUserScore=TEXT("0");
	int nNumberCount=0;
	int count=0;
	//if(m_lStorageGold/1000==0) return;
	while(lLeaveScore>0)
	{
		strTmpNumber1.Format(TEXT("%I64d"),lLeaveScore%10);
		nNumberCount++;
		...
		if(nNumberCount==3)
		{
			...
			if(count==1)
			{
				strTmpNumber2 +=strUserScore;
			}
			else
			{
				strTmpNumber2 += (TEXT(",") +strUserScore);
			}
			strUserScore=strTmpNumber2;
			nNumberCount=0;
			strTmpNumber2=TEXT("");
		}
		lLeaveScore/=10;
	...
	{
		if(strUserScore.IsEmpty()==FALSE)
			strTmpNumber2 += (TEXT(",") +strUserScore);
		strUserScore=strTmpNumber2;
	}

	return strUserScore.GetBuffer(0);

}

void CGameClientView::DrawUserTimer( CDC * pDC, int nXPos, int nYPos, WORD wTime, WORD wTimerArea/*=99*/ )
{
	//获取属性
