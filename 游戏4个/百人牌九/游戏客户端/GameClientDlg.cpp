...
BEGIN_MESSAGE_MAP(CGameClientDlg, CGameFrameDlg)
	...
	ON_MESSAGE(IDM_SICE_ONE,OnSiceOne)
...
{
	//设置标题
	SetWindowText(TEXT("牌九游戏  --  Ver：6.6.1.0"));
...
bool CGameClientDlg::OnGameMessage(WORD wSubCmdID, const void * pBuffer, WORD wDataSize)
{
	...
	case SUB_S_CANCEL_CAN:
		{
			return true;
			//return OnblSubCancelBanker(pBuffer,wDataSize);
		}
		/*   case SUB_S_APPLY_FAIL:
		{
		return true;
		}*/
	}
	...
	case GS_FREE:			//空闲状态
		{
			...
			m_GameClientView.SetSurplusCardCount(pStatusFree->cbLeftCardCount,pStatusFree->cbTableHavaSendCardArray,pStatusFree->cbTableHavaSendCount,false);
	...
	case GS_GAME_END:		//结束状态
		{
			...
			m_GameClientView.EnableSysBanker(m_bEnableSysBanker);
			m_GameClientView.SetSurplusCardCount(pStatusPlay->cbLeftCardCount,pStatusPlay->cbTableHavaSendCardArray,pStatusPlay->cbTableHavaSendCount,false);
...
bool CGameClientDlg::OnSubGameFree(const void * pBuffer, WORD wDataSize)
{
	...
	/*for (int i = 0;i<4;i++)
	{
	if(m_GameClientView.m_cbTableHavaSendCount[i]==8)
	{
	m_GameClientView.m_cbTableHavaSendCount[i] = 0;

	m_GameClientView.m_bcSurplusCardCount = 32;
	}
	}*/
...
//bool  CGameClientDlg::OnblSubCancelBanker(const void * pBuffer, WORD wDataSize)
//{
...
bool CGameClientDlg::OnSubGameEnd(const void * pBuffer, WORD wDataSize)
{
	...
	m_GameClientView.SetSurplusCardCount(pGameEnd->cbLeftCardCount,pGameEnd->cbTableHavaSendCardArray,pGameEnd->cbTableHavaSendCount);
...
//第一次摇色子消息
LRESULT CGameClientDlg::OnSiceOne(WPARAM wParam, LPARAM lParam)
{
	//环境处理
		...
		case GS_PLACE_JETTON:
			{
				...
				else
				{
					...
					{
						m_GameClientView.m_DlgBank.AllowStorage(FALSE);
						m_GameClientView.m_DlgBank.AllowDraw(FALSE);
					}
