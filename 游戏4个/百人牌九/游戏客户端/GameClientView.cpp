...
#define  IDI_SAIZI     107
#define  IDI_SAIZIRESULT 108
#define  IDI_SAIZWAIT 109
...
CGameClientView::CGameClientView() : CGameFrameView(true,24)
{
	...
	m_bcSurplusCardCount= 0;
	m_bcOrtSurplusCardCount = 0;

	memset(m_cbTableHavaSendCount,0,sizeof(m_cbTableHavaSendCount));
	...
	m_Plate.SetLoadInfo(IDB_SICE_BACK,hInstance);

	for (int i = 0;i<6;i++)
	{
		m_PlateSizeFirst[i].SetLoadInfo(IDB_SICE1+i,hInstance);
		m_PlateSizeNext[i].SetLoadInfo(IDB_SICE7+i,hInstance);

	}
	if(m_PngMinCard.IsNull())
		m_PngMinCard.LoadImage(hInstance,TEXT("IDB_GAME_MIN_PNG"));

	if(!m_PngMinCard.IsNull())
	{
		m_CardSize.SetSize(m_PngMinCard.GetWidth()/11,m_PngMinCard.GetHeight()/2);
	}
	...
	for (int i = 0;i<15;i++)
	{
		_snprintf(szBuffer,CountArray(szBuffer),TEXT("OPEN_DICEBOX_%d"),i+1);
		m_PngOPenBox[i].LoadImage(hInstance,szBuffer);
	}
	for (int i = 0;i<21;i++)
	{
		_snprintf(szBuffer,CountArray(szBuffer),TEXT("SHAKE_DICEBOX_%d"),i+1);
		m_PngSHAKEBox[i].LoadImage(hInstance,szBuffer);
	}
	m_PngDICE.LoadImage(hInstance,TEXT("DICE_15"));
	m_cbTableCardArray[BANKER_INDEX][0]=0x17;
	m_cbTableCardArray[BANKER_INDEX][1]=0x14;
	m_cbTableCardArray[SHUN_MEN_INDEX][0]=0x33;
	m_cbTableCardArray[SHUN_MEN_INDEX][1]=0x18;
...
void CGameClientView::SetSurplusCardCount(BYTE bcCardCount,BYTE cbTableHavaSendCardArray[4][8],BYTE cbTableHavaSendCount[4],bool blDes )
{
	m_bcSurplusCardCount = bcCardCount;
	m_bcOrtSurplusCardCount = m_bcSurplusCardCount;
	//CopyMemory(m_cbTableHavaSendCardArray,cbTableHavaSendCardArray,CountArray(cbTableHavaSendCardArray));
	CopyMemory(m_cbTableHavaSendCount,cbTableHavaSendCount,CountArray(cbTableHavaSendCount));
	CopyMemory(m_cbOrtTableHavaSendCount,cbTableHavaSendCount,CountArray(cbTableHavaSendCount));
	for (int i = 0;i<4;i++)
	{
		CopyMemory(m_cbTableHavaSendCardArray[i],cbTableHavaSendCardArray[i],CountArray(cbTableHavaSendCardArray[i]));
		if(blDes)
...
void CGameClientView::SetMoveCardTimer()
{
	...
	m_OpenCardIndex = 1;
	m_bcShowCount = 0;
	m_PostStartIndex =  m_GameLogic.GetCardValue(m_bcfirstShowCard+m_bcNextShowCard);
	...
	SetTimer(IDI_SAIZI,100,NULL);
	SetTimer(IDI_DISPATCH_CARD,23000,NULL);
	m_ShowSaiziIndex= 0;
	m_blShowSaizi= true;
}
//建立消息
...
void CGameClientView::RectifyGameView(int nWidth, int nHeight)
{
	...
	m_rcShunMen.left=nCenterX - 258;
	m_rcShunMen.top=nCenterY-227;
	...
	/*m_CardControl[0].SetBenchmarkPos(CPoint(nWidth/2,nHeight/2-355),enXLeft,enYTop);
	m_CardControl[1].SetBenchmarkPos(CPoint(nWidth/2-360,nHeight/2-152),enXLeft,enYTop);
	m_CardControl[2].SetBenchmarkPos(CPoint(nWidth/2,nHeight/2+30),enXLeft,enYTop);
	m_CardControl[3].SetBenchmarkPos(CPoint(nWidth/2+270-30,nHeight/2-152),enXLeft,enYTop);
	m_CardControl[4].SetBenchmarkPos(CPoint(nWidth/2,nHeight/2-152),enXLeft,enYTop);*/
	...
	//列表控件
	DeferWindowPos(hDwp,m_ApplyUser,NULL,nWidth/2 + 148,nHeight/2-291,216,50,uFlags);

	//筹码按钮
	CRect rcJetton;
	m_btJetton100.GetWindowRect(&rcJetton);
	int nYPos = nHeight/2+175;
	int nXPos = nWidth/2-64;
	int nSpace = 3;
	...
	//上庄按钮
	DeferWindowPos(hDwp,m_btApplyBanker,NULL,nWidth/2+277,nHeight/2-340,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btCancelBanker,NULL,nWidth/2+277,nHeight/2-340,0,0,uFlags|SWP_NOSIZE);

	DeferWindowPos(hDwp,m_btScoreMoveL,NULL,nWidth/2-291,nHeight/2+262,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btScoreMoveR,NULL,nWidth/2+334,nHeight/2+260,0,0,uFlags|SWP_NOSIZE);

	//开牌按钮
	DeferWindowPos(hDwp,m_btAutoOpenCard,NULL,nWidth/2+170,nHeight/2+143,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btOpenCard,NULL,nWidth/2+270,nHeight/2+143,0,0,uFlags|SWP_NOSIZE);

	//其他按钮
	DeferWindowPos(hDwp,m_btBank,NULL,nWidth/2-345,nHeight/2+143,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btContinueCard,NULL,nWidth/2-220,nHeight/2+143,0,0,uFlags|SWP_NOSIZE);

	//其他按钮
	DeferWindowPos(hDwp,m_btBankerStorage,NULL,nWidth/2-355+60,nHeight/2+141,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btBankerDraw,NULL,nWidth/2-415+50,nHeight/2+141,0,0,uFlags|SWP_NOSIZE);

#ifdef __BANKER___
	m_btBankerDraw.GetWindowRect(&rcJetton);
	DeferWindowPos(hDwp,m_btBankerStorage,NULL,nWidth/2-138,nHeight/2+177,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btBankerDraw,NULL,nWidth/2-138,nHeight/2+203,0,0,uFlags|SWP_NOSIZE);
#endif
...
void CGameClientView::DrawGameView(CDC * pDC, int nWidth, int nHeight)
{
	...
	if (wUserTimer!=0) DrawUserTimer(pDC,nWidth/2-323,nHeight/2+197,wUserTimer);
	...
	DrawSurplusCard(pDC,nWidth,nHeight);
	...
	{
		{
			if(m_ShowSaiziIndex>=21)
			{
				if(m_ShowSaiziIndex>=15+20)
				{
					m_ShowSaiziIndex = 15+20;
				}
		
				m_PngOPenBox[m_ShowSaiziIndex-21].DrawImage(pDC,nWidth/2-80-120,nHeight/2-100-30-207,m_PngOPenBox[m_ShowSaiziIndex-21].GetWidth(),
					m_PngOPenBox[m_ShowSaiziIndex-21].GetHeight(),0,0,m_PngOPenBox[m_ShowSaiziIndex-21].GetWidth(),m_PngOPenBox[m_ShowSaiziIndex-21].GetHeight());

				if(m_ShowSaiziIndex>=28)
				{
					if(m_ShowSaiziIndex>30)
					m_PngDICE.DrawImage(pDC,nWidth/2-80+180-15-120,nHeight/2-100-30+125-30-207,m_PngDICE.GetWidth()/6,
						m_PngDICE.GetHeight(),m_PngDICE.GetWidth()/6*(m_bcfirstShowCard-1),0,m_PngDICE.GetWidth()/6,m_PngDICE.GetHeight()) ;

					m_PngDICE.DrawImage(pDC,nWidth/2-80+180+20-120,nHeight/2-100-30+40+125-30-15-207,m_PngDICE.GetWidth()/6,
						m_PngDICE.GetHeight(),m_PngDICE.GetWidth()/6*(m_bcNextShowCard-1),0,m_PngDICE.GetWidth()/6,m_PngDICE.GetHeight()) ;
				}
			}
				
			else
			{
				m_PngSHAKEBox[m_ShowSaiziIndex].DrawImage(pDC,nWidth/2-80-120,nHeight/2-100-30-207,m_PngSHAKEBox[m_ShowSaiziIndex].GetWidth(),
					m_PngSHAKEBox[m_ShowSaiziIndex].GetHeight(),0,0,m_PngSHAKEBox[m_ShowSaiziIndex].GetWidth(),m_PngSHAKEBox[m_ShowSaiziIndex].GetHeight());
...
void CGameClientView::DrawNumberStringWithSpace(CDC * pDC, LONGLONG lNumber, INT nXPos, INT nYPos)
{
	...
	while (lNumber>0)
	{
		strTmpNumber1.Format(TEXT("%d"),int(lNumber%10));
		nNumberCount++;
...
void CGameClientView::OnTimer(UINT nIDEvent)
{
	...
	if(IDI_SAIZI==nIDEvent)
	{
		m_ShowSaiziIndex++;
		if(m_ShowSaiziIndex>=20)
		{
			KillTimer(IDI_SAIZI);
			SetTimer(IDI_SAIZWAIT,1000,NULL);	
		}
		AfxGetMainWnd()->PostMessage(IDM_SICE_ONE,NULL,NULL);
		UpdateGameView(NULL);
	

	}
	if(IDI_SAIZWAIT == nIDEvent)
	{
		KillTimer(IDI_SAIZWAIT);
		SetTimer(IDI_SAIZIRESULT,100,NULL);
	}
	if(IDI_SAIZIRESULT==nIDEvent)
	{
		m_ShowSaiziIndex++;
		if(m_ShowSaiziIndex>=21+15)
		{
			
			KillTimer(IDI_SAIZIRESULT);
		...
		}else
		{
		...
		
			{
				...
				for (int j = 0;j<2;j++)
				{
							...
							if(m_bcSurplusCardCount>0)
								m_bcSurplusCardCount = m_bcSurplusCardCount-1;

							if(m_bcSurplusCardCount<m_bcOrtSurplusCardCount-8)
							{
								m_bcSurplusCardCount = m_bcOrtSurplusCardCount-8; 
	...
	else if (nIDEvent==IDI_DISPATCH_CARD)
	{
		...
		CopyMemory(m_cbTableHavaSendCount ,m_cbOrtTableHavaSendCount,CountArray(m_cbOrtTableHavaSendCount));
		//更新界面
		UpdateGameView(NULL);
...
void CGameClientView::SetDispatchCardTip(enDispatchCardTip DispatchCardTip)
{
	...
	if (enDispatchCardTip_Dispatch==m_enDispatchCardTip)
	{
		for (int i = 0;i<4;i++)
		{
			//if(m_cbTableHavaSendCount[i]==8)
			{
				m_cbTableHavaSendCount[i] = 0;

				m_bcSurplusCardCount = 32;
			}
		}
...
void CGameClientView::SetFirstShowCard(BYTE bcCard[],int iCount)
{
	m_bcfirstShowCard =  bcCard[0];
	m_bcNextShowCard = bcCard[1];
...
void CGameClientView::DrawMeInfo(CDC *pDC,int nWidth,int nHeight)
{
	...
	if ( pMeUserData != NULL )
	{
		...
		//设置位置
		CRect static rcDraw;
		rcDraw.left=nWidth/2-218;
		rcDraw.top=nHeight/2+178;
...
void CGameClientView::DrawSurplusCard(CDC * pDC, int nWidth, int nHeight)
{

	ASSERT(m_bcSurplusCardCount<=32);

	int add = 0;
	for (WORD i=0;i<m_bcSurplusCardCount;i++)
	{
		if(i<16)
			m_PngMinCard.DrawImage(pDC,nWidth/2-200+m_CardSize.cx*i+122,nHeight/2-135-add,m_CardSize.cx,m_CardSize.cy,m_CardSize.cx*11,m_CardSize.cy);
		else
			m_PngMinCard.DrawImage(pDC,nWidth/2-200+m_CardSize.cx*(i-16)+122,nHeight/2+20-135-add,m_CardSize.cx,m_CardSize.cy,m_CardSize.cx*11,m_CardSize.cy);

	}

	//if(m_blMoveFinish)
	//memset(m_cbTableHavaSendCount,2,sizeof(m_cbTableHavaSendCount));
	for (int j = 0;j<4;j++)
	{
		
		if(m_cbTableHavaSendCount[j])
		{
			for (WORD i=0;i<m_cbTableHavaSendCount[j];i++)
			{
				/*int ix = 0;
				int iy = 0;
				switch (j)
				{
				case 0:
					ix =nWidth/2-100+m_CardSize.cx*(i-16)+177-152; 
					iy = nHeight/2-180-72-89;
					break;
				case 1:
					ix =nWidth/2-285+m_CardSize.cx*(i-16)+79; 
					iy = nHeight/2-180-58;

					break;
				case 2:
					ix =nWidth/2-374+m_CardSize.cx*(i-16)+167; 
					iy = nHeight/2+184-46;
					break;
				case 3:
					ix =nWidth/2+265+m_CardSize.cx*(i-16)+159; 
					iy = nHeight/2-180-59+2;
					break;
				}*/
				...
				{
				case 3:
					ix =nWidth/2-100+m_CardSize.cx*(i-16)+177-152+449+50; 
					iy = nHeight/2-180-72-89+460;
					break;
				case 1:
					ix =nWidth/2-285+m_CardSize.cx*(i-16)+79; 
					iy = nHeight/2-180-58;
				...
				case 2:
					ix =nWidth/2-374+m_CardSize.cx*(i-16)+167; 
					iy = nHeight/2+184-46-18;
					break;
				case 0:
					ix =nWidth/2+265+m_CardSize.cx*(i-16)+159-500; 
					iy = nHeight/2-180-59+2-100;
					break;
				}
				...
				CPoint Point;
				m_GameLogic.GetCardPoint(m_cbTableHavaSendCardArray[j][i],Point,m_CardSize.cx+1,m_CardSize.cy);
				if(i<16)
					m_PngMinCard.DrawImage(pDC,ix,iy,m_CardSize.cx,m_CardSize.cy,Point.x,Point.y);
				else
					m_PngMinCard.DrawImage(pDC,nWidth/2-200+m_CardSize.cx*(i-16)+122,nHeight/2+20-135-90,m_CardSize.cx,m_CardSize.cy,m_CardSize.cx*11,m_CardSize.cy);
