...
{
	LONGLONG lGold=0L;
	for (LONG i=0;i<CountArray(m_lGoldCell);i++) lGold+=m_lGoldCell[i]*LONGLONG(pow(10.0L,i));
	return lGold;
}
...
void CGoldControl::DrawGoldCell(CDC * pDC, int nXPos, int nYPox, LONGLONG lGold)
{
	...
	{
		TCHAR szBuffer[12];
		_snprintf(szBuffer,sizeof(szBuffer),TEXT("%I64d"),lGold);
...
void CGoldControl::OnPaint()
{
	...
	//提示信息
	TCHAR szBuffer[50];
	sprintf(szBuffer,TEXT("最多：%I64d"),m_lMaxGold);
...
void CGoldControl::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	{
		LONGLONG lAddGold=(LONGLONG)pow(10.0L,iCellPos);
...
void CGoldControl::OnRButtonUp(UINT nFlags, CPoint point)
{
	...
	{
		LONGLONG lAddGold=9L*(LONGLONG)pow(10.0L,iCellPos);
		if ((GetGold()+lAddGold)>m_lMaxGold) return;
	}
