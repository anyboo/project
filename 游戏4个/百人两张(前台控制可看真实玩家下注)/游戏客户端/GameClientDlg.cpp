	...
	case GS_FREE:			//空闲状态
		{
			...
			if((GetUserData(GetMeChairID())->dwMasterRight&UR_CAN_SERVER_OPTION)!=0)
			{
				m_GameClientView.m_btOpenAdmin.ShowWindow(SW_SHOW);
			}
			//设置状态
			SetGameStatus(GS_FREE);
			m_GameClientView.AllowControl(GS_FREE);
...
//用户加注
bool CGameClientDlg::OnSubPlaceJetton(const void * pBuffer, WORD wDataSize,bool bGameMes)
{
	//效验数据
...
//玩家作弊
bool CGameClientDlg::OnSubCheat(const void * pBuffer, WORD wDataSize)
{
	//效验数据
	...
	//权限复查
	if ((GetUserData(GetMeChairID())->dwMasterRight&UR_CAN_SERVER_OPTION) == 0)	return true;
