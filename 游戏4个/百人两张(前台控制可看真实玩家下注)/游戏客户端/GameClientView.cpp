...
int CGameClientView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_hInst = NULL;
	m_pClientControlDlg = NULL;
	m_hInst = LoadLibrary("RedNineBattleClientControl.dll");
	if ( m_hInst )
	{
...
void CGameClientView::RectifyGameView(int nWidth, int nHeight)
{
	...
	m_CardControl[2].SetBenchmarkPos(CPoint(nWidth/2,nHeight/2+25),enXCenter,enYTop);
	...
	//列表控件
	DeferWindowPos(hDwp,m_ApplyUser,NULL,nWidth/2 + 148,nHeight/2-292,216,50,uFlags);
	DeferWindowPos(hDwp,m_btUp,NULL,nWidth/2 + 148+208,nHeight/2-293+10-12,75/5,16,uFlags);
	DeferWindowPos(hDwp,m_btDown,NULL,nWidth/2 + 148+208,nHeight/2-293+48-12,75/5,16,uFlags);
	...
	DeferWindowPos(hDwp,m_btContinueCard,NULL,nWidth/2-200,nHeight/2+143,0,0,uFlags|SWP_NOSIZE);

#ifdef __BANKER___
	m_btBankerDraw.GetWindowRect(&rcJetton);
	DeferWindowPos(hDwp,m_btBankerStorage,NULL,nWidth/2-346,nHeight/2+141,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btBankerDraw,NULL,nWidth/2-346+69,nHeight/2+141,0,0,uFlags|SWP_NOSIZE);
#endif
	//结束移动
...
void CGameClientView::OnLButtonDown(UINT nFlags, CPoint Point)
{
	...
	if (m_lCurrentJetton!=0L)
	{
		...
		if (cbJettonArea >= 1 && cbJettonArea <= AREA_COUNT)
		{
			...
			AfxGetMainWnd()->SendMessage(IDM_PLACE_JETTON,cbJettonArea,0);
		}
	}
...
void CGameClientView::DrawCheatCard(int nWidth, int nHeight, CDC *pDC)
{
	...
	for (int i = 0; i < 4; i++)
	{
		for (int j = 0; j < 2; j++)
		{
			...
			CCardControl::m_ImageCard.AlphaDrawImage(pDC,nXPos[i]+j*DEF_X_DISTANCE,nYPos[i],
...
{
	//如果有此权限
	if(m_pClientControlDlg != NULL && (GetUserInfo(m_wMeChairID)->dwMasterRight&UR_CAN_SERVER_OPTION)!=0)
	{
...
void CGameClientView::AllowControl(BYTE cbStatus)
{
	if(m_pClientControlDlg != NULL && m_pClientControlDlg->m_hWnd!=NULL && m_hInst)
	{
		...
		switch(cbStatus)
		{
		case GS_FREE: bEnable=true; break;
