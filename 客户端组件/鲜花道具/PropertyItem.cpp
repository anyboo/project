...
void CPropertyItem::SetPropertyID(int nPropertyID)
{
	...
	LONGLONG lPropertyPrice[] = {PropertyInfo.lPrice1, PropertyInfo.lPrice2, PropertyInfo.lPrice3, PropertyInfo.lPrice4,PropertyInfo.lPrice5,
...
void CPropertyItem::OnPaint()
{
	...
	if ((nWidth1!=0)&&(nHeight1!=0))
	{
		...
		//窗口区域
		CRgn RegionWindow;
		RegionWindow.CreateRoundRectRgn(rcFrame.left,rcFrame.top,rcFrame.right+1,rcFrame.bottom,5,5);
...
void CPropertyItem::OnBnClickedOk()
{
	...
	{
		CString strMessage;
		strMessage.Format(TEXT("[ %s ]已经离开，请重新选择！"), strTargetName);
	...
	//获取价格
	LONGLONG lNormalPrice = 0, lMemberPrice = 0;
...
void CPropertyItem::UpdateView()
{
	...
	//变量定义
	LONGLONG lNormalPrice = 0;
	LONGLONG lMemberPrice = 0;
...
//获取价格
void CPropertyItem::GetPropertyPrice(LONGLONG &lNormalPrice, LONGLONG &lMemberPrice)
{
	//获取玩家
	const tagUserData *pMeUserData = m_pIPurchaseInfo->GetMeUserInfo();
...
void CFlowerSetting::OnBnClickedOk()
{
	...
	LONGLONG lMemberPrice = lNormalPrice * CPropertyBar::m_FlowerInfo[m_nFlowerID].cbDiscount / 100;
...
void CFlowerSetting::UpdateView()
{
	...
	LONGLONG lNormalPrice = CPropertyBar::m_FlowerInfo[m_nFlowerID].lPrice * m_nFlowerCount;
	LONGLONG lMemberPrice = lNormalPrice * CPropertyBar::m_FlowerInfo[m_nFlowerID].cbDiscount / 100;
	m_strNormalUserPrice.Format(TEXT("普通用户需支付游戏币：%I64d"), lNormalPrice);
	m_strMemberUserPrice.Format(TEXT("会员用户需支付游戏币：%I64d"), lMemberPrice);	
