...
void CPropertyBar::RectifyControl(int nWidth, int nHeight)
{
	...
	//按钮个数
	INT nAllLong = rcClient.Width()-(rcMoveButClient.Width()*2);
	m_nButtonCount = nAllLong / (rcButtonClient.Width()+BUTTON_DISTANCE_X+2);
	const int nButtonCount=m_nButtonCount;

	//起始位置
	m_nXoriginal =(nAllLong-m_nButtonCount*(rcButtonClient.Width()+BUTTON_DISTANCE_X))/2;
...
// 
// 	//左右按钮
// 	DeferWindowPos(hDwp, m_btTopMoveToR, NULL, rcClient.Width() - 18-rcMoveButClient.Width()*2, 18, 0, 0, uFlags);
// 	DeferWindowPos(hDwp, m_btTopMoveToL, NULL, rcClient.Width() - rcMoveButClient.Width()-10, 18, 0, 0, uFlags);
// 	DeferWindowPos(hDwp, m_btBtmMoveToL, NULL, rcClient.Width() - rcMoveButClient.Width()-10, 53, 0, 0, uFlags);
// 	DeferWindowPos(hDwp, m_btBtmMoveToR, NULL, rcClient.Width() - 18-rcMoveButClient.Width()*2, 53, 0, 0, uFlags);
// 
