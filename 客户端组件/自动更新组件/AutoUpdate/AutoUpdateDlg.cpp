...
//#endif
	...
	//读取配置参数
	strSiteURL = AfxGetApp()->GetProfileString(_T("Download"), _T("site1"), _T("http://www.7x78.com/down"));
	strTempDir = AfxGetApp()->GetProfileString(_T("Setup"), _T("TempDir"), _T("Update"));
//	AfxGetApp()->WriteProfileString(_T("Download"), _T("site1"), _T("http://www.55uc.com/down"));
	//设置URL路径
...
void CAutoUpdateDlg::OnStatusCallback(ULONG ulProgress, ULONG ulProgressMax, ULONG ulStatusCode, LPCTSTR pszStatusText)
{
	...
	case DMS_ENDSETUP:
		{
			...
			strcat(szModuleName, _T("\\7x78Game.exe -noupdate"));
	...
	case DMS_ERROR:
		{
			CString strError = AfxGetApp()->GetProfileString(_T("messages"), _T("ErrorMsg"), _T("更新失败，请到www.7x78.com下载最新版本"));
