...
VOID CPasswordControl::PreSubclassWindow()
{
	...
	m_btKeyboard.SetButtonImage(IDB_BT_KEYBOARD,GetModuleHandle(CLIENT_SHARE_DLL_NAME),false);
...
//获取密码
LPCTSTR CPasswordControl::GetUserPassword(TCHAR szPassword[32])
{
	//控件密码
	if (m_bModify==true)
	{
	...
	//拷贝密码
	lstrcpyn(szPassword,m_szPassword,32);
...
VOID CPasswordControl::OnNcPaint()
{
	...
	COLORREF crFrameBorad=RGB(92,100,105);
	...
	if (IsWindowEnabled()==FALSE)
	{
		...
		{
			crBackGround=RGB(125,125,125);;
			crFrameBorad=GetSysColor(COLOR_WINDOW);
		}
		else
		{
			crBackGround=RGB(125,125,125);;
...
CPasswordKeyboard::CPasswordKeyboard() : CDialog(IDD_PASSWORD_KEYBOARD)
{
	...
	//变量定义
	HINSTANCE hResInstance=GetModuleHandle(CLIENT_SHARE_DLL_NAME);
...
//	CSkinRenderManager * pSkinRenderManager=CSkinRenderManager::GetInstance();
	...
	//CSkinRenderManager pSkinRenderManager;
	//pSkinRenderManager.GetInstance();
...
BOOL CPasswordKeyboard::OnSetCursor(CWnd * pWnd, UINT nHitTest, UINT uMessage)
{
	...
	{
		SetCursor(LoadCursor(GetModuleHandle(CLIENT_SHARE_DLL_NAME),MAKEINTRESOURCE(IDC_HAND_CUR)));
		return true;
	}
