...
BEGIN_MESSAGE_MAP(CDownLoadMission, CSkinDialogEx1)
	ON_WM_TIMER()
	ON_WM_PAINT()
	...
	case enDownLoadStatus_DownLoadIng:
		{
			...
			TCHAR szTemp[256] ={0};
			lstrcpyn(szTemp, m_pDownLoadRequest->szFileName, 256);
			...
			_snprintf(DownLoadStatus.szStatus,sizeof(DownLoadStatus.szStatus),
				...
				TEXT("%s更新了%ld%%..."), szTemp, (m_dwDownLoadSize*100/m_dwTotalFileSize));
			break;
		}
...
//构造函数
CDownLoadMission::CDownLoadMission(IDownLoadMissionSink * pIDownLoadMissionSink) : CSkinDialogEx1(IDD_DOWN_LOAD)
{
	//设置变量
	m_dwMissionID=0;
	m_dwDownLoadType=0;
...
BOOL CDownLoadMission::OnInitDialog()
{
	...
	//设置定时器
	SetTimer(IDI_UPDATE_TIMER,10,NULL);

	HINSTANCE hInstancle=GetModuleHandle(IDOWN_LOAD_DLL_NAME);
	m_btCancel.SetButtonImage(IDB_BTN_CANCLE, hInstancle, true);
...
void CDownLoadMission::OnPaint()
{
	...
	HINSTANCE hInstancle=GetModuleHandle(IDOWN_LOAD_DLL_NAME);
	CImage ImageBack;
	ImageBack.LoadFromResource(hInstancle,IDB_DOWNBACK);

	CImage	ImageProcess;
	ImageProcess.LoadFromResource(hInstancle, IDB_DOWNPROCESS);

	int nWidth=ImageBack.GetWidth();
	int nHeight=ImageBack.GetHeight();
	//调整判断
	...
	//绘画背景
	CImage ImageBuffer;
	ImageBuffer.Create(ImageBack.GetWidth(),ImageBack.GetHeight(),16);
	CDC * pDCBuffer=CDC::FromHandle(ImageBuffer.GetDC());

	//绘画状态
	CDC * pDCImage=CDC::FromHandle(ImageBack.GetDC());
	AlphaDrawImage(pDCBuffer,0,0,ImageBack.GetWidth(),ImageBack.GetHeight(),pDCImage,0,0,ImageBack.GetWidth(),ImageBack.GetHeight(),RGB(255,0,255));

	tagDownLoadStatus DownLoadStatus;
	m_DownLoadThread.GetDownLoadStatus(DownLoadStatus);
	int nStype= ImageProcess.GetWidth()/100;
	int nPos = nStype* DownLoadStatus.wProgress;
	CDC * pDCImage1=CDC::FromHandle(ImageProcess.GetDC());
	AlphaDrawImage(pDCBuffer,6,74,nPos,ImageProcess.GetHeight(),pDCImage1,0,0,nPos,ImageProcess.GetHeight(),RGB(255,0,255));

	//绘画消息
	CFont DrawFont;
	DrawFont.CreateFont(-14,0,0,0,500,0,0,0,134,3,2,1,2,TEXT("宋体"));
	CFont * pOldFont=pDCBuffer->SelectObject(&DrawFont);
	pDCBuffer->SetTextColor(RGB(101,62,29));
	pDCBuffer->SetBkMode(TRANSPARENT);
	pDCBuffer->TextOut(15,40,DownLoadStatus.szStatus);
