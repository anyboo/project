...
{
	//显示消息
	if (pRecvUserData==NULL) m_GameFrameDlg.m_MessageProxyHelper->InsertUserChat(pSendUserData->szGroupName,pszChatMessage,crColor,MS_NORMAL);
	else m_GameFrameDlg.m_MessageProxyHelper->InsertUserChat(pSendUserData->szName,pRecvUserData->szGroupName,pszChatMessage,crColor,MS_NORMAL);
...
void __cdecl CClientKernelSink::OnEventUserEnter(tagUserData * pUserData, WORD wChairID, bool bLookonUser)
{
	...
	{
		//插入用户
		int nItem=m_GameFrameDlg.m_pGameFrameControl->m_ChatObject.AddString(pUserData->szGroupName);
		m_GameFrameDlg.m_pGameFrameControl->m_ChatObject.SetItemData(nItem,pUserData->dwUserID);

		//提示消息
		if (m_GameFrameDlg.m_bGameSceen==true)
		{
			m_GameFrameDlg.m_MessageProxyHelper->InsertUserName(pUserData->szGroupName);
