...
CImageList		CUserListView::m_GenderImage;		
CImageList		CUserListView::m_MemberImage;		
//排序信息结构
...
#define		COLOR_TEXT_NORMAL	RGB(244,241,222)
...
bool CUserListView::InitUserListView()
{
	...
	//设置颜色
	SetTextColor(RGB(247,242,223));
	SetBkColor(RGB(35,35,35));
	SetTextBkColor(RGB(35,35,35));
	...
	{
		m_wColumnCount=18;
		InsertColumn(0,TEXT(" "),LVCFMT_CENTER,20);
		InsertColumn(1,TEXT(" "),LVCFMT_CENTER,20);
		InsertColumn(2,TEXT(" "),LVCFMT_CENTER,20);
		InsertColumn(3,TEXT("昵称"),LVCFMT_CENTER,75);
		InsertColumn(4,TEXT("ID"),LVCFMT_CENTER,40);
		InsertColumn(5,TEXT("欢乐豆"),LVCFMT_CENTER,60);
		InsertColumn(6,TEXT(""/*"魅力"*/),LVCFMT_CENTER,0);
		InsertColumn(7,TEXT("用户等级"),LVCFMT_CENTER,85);
		InsertColumn(8,TEXT("经验值"),LVCFMT_CENTER,50);
		InsertColumn(9,TEXT("总局"),LVCFMT_CENTER,60);
		InsertColumn(10,TEXT("胜局"),LVCFMT_CENTER,60);
		InsertColumn(11,TEXT("输局"),LVCFMT_CENTER,60);
		InsertColumn(12,TEXT("和局"),LVCFMT_CENTER,60);
		InsertColumn(13,TEXT("逃局"),LVCFMT_CENTER,60);
		InsertColumn(14,TEXT("胜率"),LVCFMT_CENTER,60);
		InsertColumn(15,TEXT("逃率"),LVCFMT_CENTER,60);
		InsertColumn(16,TEXT("个性签名"),LVCFMT_CENTER,200);
		InsertColumn(17,TEXT("旁观用户"),LVCFMT_CENTER,90);

		m_wDataDescribe[0]=1000;
		m_wDataDescribe[1]=1001;
		m_wDataDescribe[2]=1002;
		m_wDataDescribe[3]=DTP_USER_ACCOUNTS;
		m_wDataDescribe[4]=DTP_GAME_ID;
		m_wDataDescribe[5]=DTP_USER_SCORE;
		m_wDataDescribe[6]=DTP_LOVE_LINESS;
		m_wDataDescribe[7]=DTP_GAME_LEVEL;
		m_wDataDescribe[8]=DTP_USER_EXPERIENCE;
		m_wDataDescribe[9]=DTP_USER_PLAY_COUNT;
		m_wDataDescribe[10]=DTP_USER_WIN;
		m_wDataDescribe[11]=DTP_USER_LOST;
		m_wDataDescribe[12]=DTP_USER_DRAW;
		m_wDataDescribe[13]=DTP_USER_FLEE;
		m_wDataDescribe[14]=DTP_WIN_RATE;
		m_wDataDescribe[15]=DTP_FLEE_RATE;
		m_wDataDescribe[16]=DTP_UNDER_WRITE;
		m_wDataDescribe[17]=DTP_LOOK_ON_USER;

	}
	else
	{
		m_wColumnCount=18;										//	m_wColumnCount=16;
		InsertColumn(0,TEXT(" "),LVCFMT_CENTER,20);
		InsertColumn(1,TEXT(" "),LVCFMT_CENTER,20);
		InsertColumn(2,TEXT(" "),LVCFMT_CENTER,20);
		InsertColumn(3,TEXT("昵称"),LVCFMT_CENTER,75);			//	InsertColumn(0,TEXT("用户名"),LVCFMT_CENTER,115);	
		InsertColumn(4,TEXT("ID"),LVCFMT_CENTER,40);			//	InsertColumn(1,TEXT("游戏 ID"),LVCFMT_CENTER,60);
		InsertColumn(5,TEXT("欢乐豆"),LVCFMT_CENTER,60);			//	InsertColumn(2,TEXT("积分"),LVCFMT_CENTER,60);
		InsertColumn(6,TEXT(""/*"魅力"*/),LVCFMT_CENTER,0);			//	InsertColumn(3, TEXT("银行"),LVCFMT_CENTER,60);
		InsertColumn(7,TEXT("用户等级"),LVCFMT_CENTER,85);		//	InsertColumn(4, TEXT("魅力"),LVCFMT_CENTER,60);
		InsertColumn(8,TEXT("经验值"),LVCFMT_CENTER,50);			//	InsertColumn(5, TEXT("用户等级"),LVCFMT_CENTER,85);
		InsertColumn(9,TEXT("总局"),LVCFMT_CENTER,60);			//	InsertColumn(6, TEXT("经验值"),LVCFMT_CENTER,50);
		InsertColumn(10,TEXT("胜局"),LVCFMT_CENTER,60);			//	InsertColumn(7, TEXT("总局"),LVCFMT_CENTER,60);
		InsertColumn(11,TEXT("输局"),LVCFMT_CENTER,60);			//	InsertColumn(8, TEXT("胜局"),LVCFMT_CENTER,60);
		InsertColumn(12,TEXT("和局"),LVCFMT_CENTER,60);			//	InsertColumn(9, TEXT("输局"),LVCFMT_CENTER,60);
		InsertColumn(13,TEXT("逃局"),LVCFMT_CENTER,60);			//	InsertColumn(10,TEXT("和局"),LVCFMT_CENTER,60);
		InsertColumn(14,TEXT("胜率"),LVCFMT_CENTER,60);			//	InsertColumn(11,TEXT("逃局"),LVCFMT_CENTER,60);
		InsertColumn(15,TEXT("逃率"),LVCFMT_CENTER,60);			//	InsertColumn(12,TEXT("胜率"),LVCFMT_CENTER,60);
		InsertColumn(16,TEXT("个性签名"),LVCFMT_CENTER,200);		//	InsertColumn(13,TEXT("逃率"),LVCFMT_CENTER,60);
		InsertColumn(17,TEXT("旁观用户"),LVCFMT_CENTER,90);		//	InsertColumn(14,TEXT("个性签名"),LVCFMT_CENTER,200);
																//	InsertColumn(15,TEXT("旁观用户"),LVCFMT_CENTER,90);

		m_wDataDescribe[0]=1000;
		m_wDataDescribe[1]=1001;
		m_wDataDescribe[2]=1002;
		m_wDataDescribe[3]=DTP_USER_ACCOUNTS;					//	
		m_wDataDescribe[4]=DTP_GAME_ID;							//	m_wDataDescribe[0]=DTP_USER_ACCOUNTS;
		m_wDataDescribe[5]=DTP_USER_SCORE;						//	m_wDataDescribe[1]=DTP_GAME_ID;
		m_wDataDescribe[6]=DTP_LOVE_LINESS;						//	m_wDataDescribe[2]=DTP_USER_SCORE;
		m_wDataDescribe[7]=DTP_GAME_LEVEL;						//	m_wDataDescribe[3]=DTP_USER_GOLD;
		m_wDataDescribe[9]=DTP_USER_EXPERIENCE;					//	m_wDataDescribe[4]=DTP_LOVE_LINESS;
		m_wDataDescribe[9]=DTP_USER_PLAY_COUNT;					//	m_wDataDescribe[5]=DTP_GAME_LEVEL;
		m_wDataDescribe[10]=DTP_USER_WIN;						//	m_wDataDescribe[6]=DTP_USER_EXPERIENCE;
		m_wDataDescribe[11]=DTP_USER_LOST;						//	m_wDataDescribe[7]=DTP_USER_PLAY_COUNT;
		m_wDataDescribe[12]=DTP_USER_DRAW;						//	m_wDataDescribe[8]=DTP_USER_WIN;
		m_wDataDescribe[13]=DTP_USER_FLEE;						//	m_wDataDescribe[9]=DTP_USER_LOST;
		m_wDataDescribe[14]=DTP_WIN_RATE;						//	m_wDataDescribe[10]=DTP_USER_DRAW;
		m_wDataDescribe[15]=DTP_FLEE_RATE;						//	m_wDataDescribe[11]=DTP_USER_FLEE;
		m_wDataDescribe[16]=DTP_UNDER_WRITE;					//	m_wDataDescribe[12]=DTP_WIN_RATE;
		m_wDataDescribe[17]=DTP_LOOK_ON_USER;					//	m_wDataDescribe[13]=DTP_FLEE_RATE;
...
void CUserListView::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	for (int i=0;i<iCount;i++)
	{
		...
		if ( i == 0 )
		{
			...
			else pDC->SetTextColor(COLOR_TEXT_NORMAL);

			if(0 < pIUserItem->cbMemberOrder)
			{
				m_MemberImage.Draw(pDC, pIUserItem->cbMemberOrder-1,CPoint(rcSubItem.left,rcSubItem.top),ILD_TRANSPARENT);
			}
			...
			int iGender =(int)(pIUserItem->cbGender);
...
// 			if (m_cbMaster==TRUE && pIUserItem->cbAndroid==FALSE) ///非机器人特别显示
// 			{
...
LPCTSTR CUserListView::GetDisplayText(tagUserData * pUserData, WORD wColumnPos)
{
	...
	case DTP_LOVE_LINESS:		//用户魅力
		{
			_snprintf(szBuffer,sizeof(szBuffer),TEXT("")/*,pUserData->lLoveliness*/);
			return szBuffer;
		}
...
bool CUserListView::SetMeUserData( tagUserData const* pCurrentUserData )
{
	//设置用户
	if (m_pCurrentUserData!=pCurrentUserData)
	{
		...
		if(pCurrentUserData != NULL)
			m_cbMaster = pCurrentUserData->dwMasterRight>0?TRUE:FALSE;
	}
