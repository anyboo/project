...
//绘画头像
void CGameFrameView::DrawUserFace(CDC * pDC,const tagUserData * pUserData, int nXPos, int nYPos, bool bOffLine,int X,int Y)
{
	ASSERT(m_pIUserFaceRes!=NULL && pUserData!=NULL);
	m_pIUserFaceRes->DrawNormalFace(pDC,nXPos,nYPos,pUserData->wFaceID,pUserData->dwUserID,pUserData->dwCustomFaceVer,X,Y);
	return;
}
