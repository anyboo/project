...
#define IDM_UM_LOOKON				WM_USER+109							//进入旁观
#define IDM_UM_CALLUSER				WM_USER+110							//呼叫用户
#define IDM_UM_PLAYERINFO			WM_USER+111							//玩家信息
...
#define IDM_UM_CUT_USERALL				WM_USER+157							//用户下线
...
BEGIN_MESSAGE_MAP(CRoomViewItem, CDialog)
	...
	ON_BN_CLICKED(IDC_BANK,	ShowBankDlg)
	...
	ON_BN_CLICKED(IDC_ROOM_BIND, OnBindPc)
...
CRoomViewItem::CRoomViewItem() : CDialog(IDD_GAME_ROOM), m_VorSplitter(VorSpliter), m_HorSplitter(HorSpliter),m_BankStorage(0)
{
	//辅助变量
	m_wGameGenre=0;
	m_dwVideoAddr=0L;
...
tagUserData * CRoomViewItem::SearchUserItem(LPCTSTR pszUserName)
{
	...
	while (true)
	{
		...
		//比较名字
		CString strName,strFineUser,strCroupName;
		strCroupName.Format(TEXT("%s"),pUserData->szName);
		strName.Format(TEXT("%s"),pUserData->szGroupName);
		strFineUser.Format(TEXT("%s"),pszUserName);
		if ((strName==strFineUser)||(strCroupName==strFineUser)) return pUserData;
	};
	return NULL;
}
...
void CRoomViewItem::DoDataExchange(CDataExchange * pDX)
{
	...
	DDX_Control(pDX, IDC_ROOM_SET,m_btRoomSet);
	...
	DDX_Control(pDX, IDC_ROOM_BIND, m_btBindPc);
}
...
BOOL CRoomViewItem::OnInitDialog()
{
	...
	SkinSB_Init(m_UserListView,CGameFrame::m_bmScroll);
	m_ChatInput.LimitText(MAX_CHAT_LEN-1);
	m_ChatMessage.SetBackgroundColor(FALSE,RGB(253,253,253));

	//创建控件
	m_BrowerAD.Create(NULL,NULL,WS_VISIBLE|WS_CHILD,CRect(0,0,0,0),this,100,NULL);

	m_BrowerAD.Navigate(TEXT("http://www.7x78.com/AD/RoomView.html"),NULL,NULL,NULL,NULL);

	m_BrowerAD1.Create(NULL,NULL,WS_VISIBLE|WS_CHILD,CRect(0,0,0,0),this,100,NULL);
	//m_BrowerAD1.Navigate(TEXT("http://www.7x78.com/AD/RoomRight.html"),NULL,NULL,NULL,NULL);

	//MessageBox(SS,"asdasd",NULL);
	//道具控件
	...
	case IDM_UM_PLAYERINFO:		//玩家资料
		{
			...
			GetCursorPos(&point);
			point.y-=135;
			ShowPlayerInfo(pIUserItem, point);
			return TRUE;
		}
	case IDM_UM_COPY_NAME:		//拷贝名字
		{
			...
			lstrcpy(szMemName,pIUserItem->GetUserData()->szGroupName);
	...
	case IDM_UM_LOOKON:	//进入旁观
		{
			...
			if ((pUserData->wTableID!=INVALID_TABLE)&&(pUserData->wChairID!=INVALID_CHAIR))
			{
				...
				else
				{
					...
					PerformLookonAction(pUserData->wTableID,pUserData->wChairID);
				}
			}
	...
	case IDM_UM_CALLUSER:		//呼叫用户
		{
			...
			UserChat.dwTargetUserID=pIUserItem->GetUserID();
			UserChat.dwSendUserID=pMeUserData->dwUserID;
			...
			m_ClientSocket->SendData(MDM_GR_USER,SUB_GR_CALLUSER,&UserChat,wSendSize);
	...
	case IDM_UM_CUT_USERALL:		//踢出用户
		{
			...
			_snprintf(szMessage,CountArray(szMessage),TEXT("确实要踢出全部用户吗？"));
...
void CRoomViewItem::OnPaint()
{
...

	{
		...
		for(int iL=iLeft; iL <= rcClient.Width(); iL =iL +m_ImageFrameMidT.GetWidth())
		{
			...
			if(iL > m_rcTable.left && iL < m_rcTable.right) iL++;
		
			m_ImageFrameMidT.BitBlt(dc, iL , iYPos);
			for (int iB = iYPos+m_ImageFrameMidT.GetHeight();iB < (rcClient.Height()); iB+=m_ImageFrameMidB.GetHeight())
			{
				m_ImageFrameMidB.BitBlt(dc, iL , iB);
			}
		}
...
// 
// 		CRect rcChat;
// 		rcChat.left=rcVorSplitter.right-3-5;
// 		rcChat.right=rcClient.Width();
// 		rcChat.top=rcVorSplitter.top;
...
void CRoomViewItem::OnNMDblclkUserList(NMHDR * pNMHDR, LRESULT * pResult)
{
	...
	if ((pListNotify->iItem!=-1)&&(m_cbHideUserInfo==FALSE))
	{
		...
		if ((pIUserItem!=NULL)&&(pIUserItem!=m_pMeUserItem)) 
		{
			...
			ShowPlayerInfo(pIUserItem, MousePoint);
		}
	}
...
void CRoomViewItem::OnBnClickedSendChat()
{
	...
	if((m_wGameGenre==GAME_GENRE_GOLD)&&(m_TableFrame.m_wChairCount>8)&&(m_pGameOption->m_wKindID!=546))
	{

		m_MessageProxyHelper->InsertSystemString("本房间不允许聊天",0);
		//ShowInformation(TEXT(""),0,MB_OK);
	}
	else
	{
		//聊天信息
...
void CRoomViewItem::OnBnClickedBugle()
{
	...
	if((m_wGameGenre==GAME_GENRE_GOLD)&&(m_TableFrame.m_wChairCount>8))
		m_MessageProxyHelper->InsertSystemString("本房间不允许使用小喇叭",0);	
	//ShowInformation(TEXT("本房间不允许使用小喇叭"),0,MB_OK);
	else
		//设置对话框
...
void CRoomViewItem::OnBnClickedBank()
{
	...
	m_BankStorage.SetSendInfo(NULL,m_ClientSocket.GetInterface(),pMeUserData, GlobalUserData.szPassWord);
	if (!IsWindow(m_BankStorage.m_hWnd))
	{
		m_BankStorage.CreateBankWindow(this,0);
		m_BankStorage.CenterWindow(this);
		m_BankStorage.ShowWindow(SW_SHOW);
		m_BankStorage.UpdateBank();
	}
	return;
}
...
void CRoomViewItem::OnBnClickedAutoSit()
{
	...
	//CDlgEnterTablePass dlg;
	if (m_pFindTable==NULL) 
	{
		m_pFindTable=new CDlgFindTable(this);
	}
	if (m_pFindTable->GetSafeHwnd()==NULL)
	{
		m_pFindTable->Create(IDD_FINDTABLE,this);
		//m_pDlgPlayerInfo->m_UserList.SetColumnDescribe(m_ListColumnInfo.ColumnItem,m_ListColumnInfo.wColumnCount);
	}

	m_pFindTable->ShowWindow(SW_SHOW);
	return ;
	//判断状态
...
//设置按钮
void CRoomViewItem::OnBnClickedRoomSet()
{
	//获取参数
	CGameOption * pGameOption=m_pGameOption;
	CServerOption * pServerOption=m_pServerOption;
...
// 	}
	...
	CTableOption DlgTableOption(this, pServerOption);
	if(DlgTableOption.DoModal() == IDOK)
	{
		SendUserRule();
	}
	return;
...
bool CRoomViewItem::InitRoomViewItem(CListServer * pListServer)
{
	...
	SkinSB_Init(m_ChatMessage,CGameFrame::m_bmScroll);
	//设置组件
	...
	//////////////////////////////////////////////////////////////////////////
	//0512 add 全局桌子 密码
	m_pServerOption->m_bPassword = g_GlobalOption.m_bPassword;
	lstrcpyn(m_pServerOption->m_szPassword, g_GlobalOption.m_szPassword, CountArray(m_pServerOption->m_szPassword));
	//////////////////////////////////////////////////////////////////////////
...
bool CRoomViewItem::ShowUserInfoMenu(IUserItem * pIUserItem, CPoint Point)
{
	...
	{
		//呼叫
		if ((m_cbHideUserInfo==FALSE)) 
		{
			UserInfoMenu.AppendMenu(0,IDM_UM_CALLUSER,TEXT("呼叫"));
			UserInfoMenu.SetDefaultItem(IDM_UM_CALLUSER,MF_BYCOMMAND);
		}

		//邀请游戏
		if ((bMySelf==false)&&(m_cbHideUserInfo==FALSE))
		{
			_snprintf(szBuffer,CountArray(szBuffer),TEXT("邀请"));
			UserInfoMenu.AppendMenu(0,IDM_UM_INVITE_PLAY,szBuffer);
			if((pUserData->wTableID!=INVALID_TABLE && cbUserStatus > US_READY) || ((wMeTableID==INVALID_TABLE) || (cbMeUserStatus>=US_PLAY))) UserInfoMenu.EnableMenuItem(IDM_UM_INVITE_PLAY,MF_BYCOMMAND|MF_GRAYED);
		}

		//查看位置
		if (bMySelf==false) UserInfoMenu.AppendMenu(0,IDM_UM_LOOKON,TEXT("进入旁观"));
		if (pUserData->wTableID==INVALID_TABLE || cbUserStatus <  US_PLAY) UserInfoMenu.EnableMenuItem(IDM_UM_LOOKON,MF_BYCOMMAND|MF_GRAYED);
		...
		if ((bMySelf==false)&&(m_cbHideUserInfo==FALSE))
		{
			UserInfoMenu.AppendMenu(0,IDM_UM_SET_FRIEND,TEXT("加为好友"));
		}

		UserInfoMenu.AppendMenu(MF_SEPARATOR);
	}

	UserInfoMenu.AppendMenu(0,IDM_UM_COPY_NAME,TEXT("复制用户名"));
	UserInfoMenu.AppendMenu(MF_SEPARATOR);
	UserInfoMenu.AppendMenu(0,IDM_UM_PLAYERINFO,TEXT("详细信息"));
	/*
	...
	{
	_snprintf(szBuffer,CountArray(szBuffer),TEXT("与 [ %s ] 交谈"),pUserData->szGroupName);
	...
	{
	_snprintf(szBuffer,CountArray(szBuffer),TEXT("邀请 [ %s ] 一起游戏"),pUserData->szGroupName);
	UserInfoMenu.AppendMenu(0,IDM_UM_INVITE_PLAY,szBuffer);
	}
	...
	_snprintf(szBuffer,CountArray(szBuffer),TEXT("昵称：%s [ %s ]"),pUserData->szGroupName,pszMember[cbMemberOrder]);
	}
	else _snprintf(szBuffer,CountArray(szBuffer),TEXT("昵称：%s"),pUserData->szGroupName);
	...
	if ((pMeUserData->dwUserID!=m_dwMenuUserID)&&(pMeUserData->dwMasterRight!=0L))
	{
		...
		UserInfoMenu.AppendMenu(0,IDM_UM_LOOK_USER_IP,TEXT("查看用户 IP和金币..."));
		UserInfoMenu.AppendMenu(0,IDM_UM_CUT_USER,TEXT("踢用户下线...(&T)"));
		UserInfoMenu.AppendMenu(0,IDM_UM_CUT_USERALL,TEXT("踢所有用户下线...(&X)"));
		...
		if (CUserRight::CanCutUser(dwMasterRight)==false) {UserInfoMenu.EnableMenuItem(IDM_UM_CUT_USER,MF_BYCOMMAND|MF_GRAYED);UserInfoMenu.EnableMenuItem(IDM_UM_CUT_USERALL,MF_BYCOMMAND|MF_GRAYED);}
...
// 	m_ImageTitle1.SetLoadInfo(IDB_SCORE_NUMBER1,hInstance1);
// 	m_ImageTitle2.SetLoadInfo(IDB_SCORE_NUMBER2,hInstance1);
// 	m_ImageTitle3.SetLoadInfo(IDB_SCORE_NUMBER3,hInstance1);
	//获取资源
	...
	m_ImagePropertyBar.SetLoadInfo(szResPath("Room\\Other\\PropertyBar.bmp"));
...
// 	m_EncircleList.ImageBR.SetLoadInfo(GetServerViewRes().uUserListBR,hInstance);
	...
	//列表区域
	m_EncircleList.ImageML.SetLoadInfo(szResPath("Room\\UserList\\USER_LIST_ML.bmp"));
	m_EncircleList.ImageMR.SetLoadInfo(szResPath("Room\\UserList\\USER_LIST_MR.bmp"));
	m_EncircleList.ImageTL.SetLoadInfo(szResPath("Room\\UserList\\USER_LIST_TL.bmp"));
	m_EncircleList.ImageTM.SetLoadInfo(szResPath("Room\\UserList\\USER_LIST_TM.bmp"));
	m_EncircleList.ImageTR.SetLoadInfo(szResPath("Room\\UserList\\USER_LIST_TR.bmp"));
	m_EncircleList.ImageBL.SetLoadInfo(szResPath("Room\\UserList\\USER_LIST_BL.bmp"));
	m_EncircleList.ImageBM.SetLoadInfo(szResPath("Room\\UserList\\USER_LIST_BM.bmp"));
	m_EncircleList.ImageBR.SetLoadInfo(szResPath("Room\\UserList\\USER_LIST_BR.bmp"));
...
// 	m_EncircleChat.ImageBR.SetLoadInfo(GetServerViewRes().uChatBR,hInstance);
	...
	m_EncircleChat.ImageML.SetLoadInfo(szResPath("Room\\Chat\\CHAT_ML.bmp"));
	m_EncircleChat.ImageMR.SetLoadInfo(szResPath("Room\\Chat\\CHAT_MR.bmp"));
	m_EncircleChat.ImageTL.SetLoadInfo(szResPath("Room\\Chat\\CHAT_TL.bmp"));
	m_EncircleChat.ImageTM.SetLoadInfo(szResPath("Room\\Chat\\CHAT_TM.bmp"));
	m_EncircleChat.ImageTR.SetLoadInfo(szResPath("Room\\Chat\\CHAT_TR.bmp"));
	m_EncircleChat.ImageBL.SetLoadInfo(szResPath("Room\\Chat\\CHAT_BL.bmp"));
	m_EncircleChat.ImageBM.SetLoadInfo(szResPath("Room\\Chat\\CHAT_BM.bmp"));
	m_EncircleChat.ImageBR.SetLoadInfo(szResPath("Room\\Chat\\CHAT_BR.bmp"));
...
// 	m_EncircleTable.ImageBR.SetLoadInfo(GetServerViewRes().uTableBR,hInstance);
	...
	m_EncircleTable.ImageML.SetLoadInfo(szResPath("Room\\Table\\TABLE_ML.bmp"));
	m_EncircleTable.ImageMR.SetLoadInfo(szResPath("Room\\Table\\TABLE_MR.bmp"));
	m_EncircleTable.ImageTL.SetLoadInfo(szResPath("Room\\Table\\TABLE_TL.bmp"));
	m_EncircleTable.ImageTM.SetLoadInfo(szResPath("Room\\Table\\TABLE_TM.bmp"));
	m_EncircleTable.ImageTR.SetLoadInfo(szResPath("Room\\Table\\TABLE_TR.bmp"));
	m_EncircleTable.ImageBL.SetLoadInfo(szResPath("Room\\Table\\TABLE_BL.bmp"));
	m_EncircleTable.ImageBM.SetLoadInfo(szResPath("Room\\Table\\TABLE_BM.bmp"));
	m_EncircleTable.ImageBR.SetLoadInfo(szResPath("Room\\Table\\TABLE_BR.bmp"));
...
// 	m_btRoomSet.SetButtonImage(GetServerViewRes().uBtRoomSet,hInstance,false);
// 	m_btBindPc.SetButtonImage(GetServerViewRes().uBtBindPc,hInstance, false);
...
// 	m_btBugle.SetButtonImage(GetServerViewRes().uBtBugle,hInstance,false);
	...
	m_btAutoSit.SetButtonImage(szResPath("Room\\Button\\AutoSit.bmp"),false);
	m_btBank.SetButtonImage(szResPath("Room\\Button\\Bank.bmp"),false);
	m_btFindUser.SetButtonImage(szResPath("Room\\Button\\FindUser.bmp"),false);
	m_btQuitRoom.SetButtonImage(szResPath("Room\\Button\\QuitRoom.bmp"),false);
	m_btRoomSet.SetButtonImage(szResPath("Room\\Button\\RoomSet.bmp"),false);
	m_btBindPc.SetButtonImage(szResPath("Room\\Button\\BindPc.bmp"),false);
	//m_btPhrase.SetButtonImage(GetServerViewRes().uBtCleanScreen,hInstance,false);
	m_btExpression.SetButtonImage(szResPath("Room\\Button\\Expression.bmp"),false);
	m_btColorSet.SetButtonImage(szResPath("Room\\Button\\ColorSet.bmp"),false);
	m_btCleanScreen.SetButtonImage(szResPath("Room\\Button\\CleanScreen.bmp"),false);
	m_btSendChat.SetButtonImage(szResPath("Room\\Button\\SendChat.bmp"),false);
	m_btBugle.SetButtonImage(szResPath("Room\\Button\\Bugle.bmp"),false);
...
void CRoomViewItem::DrawTableFrame(CDC * pDC)
{
	...
	//绘画框架
	CRect rcRoomTableIni(0,0, 0, 0);
	CGameFrame::GetRectFromIni(szResPath("GameFrame.ini"), TEXT("Room"), TEXT("RoomTable"), rcRoomTableIni, rcRoomTableIni);
	rcRoomTableIni.bottom = rcClient.Height()-rcRoomTableIni.bottom;
	rcRoomTableIni.right = rcClient.Width()-rcRoomTableIni.right;
	m_rcTable = rcRoomTableIni;
	CSkinAide::DrawEncircleFrame(pDC,rcRoomTableIni,m_EncircleTable);
	//CSkinAide::DrawEncircleFrame(pDC,rcRoomTableIni,m_EncircleTable);
	//CSkinAide::DrawEncircleFrame(pDC, rcRoomTableIni, m_EncircleList);

	//绘制白条
	// 	CRect rcToolBar;
	// 	rcToolBar.top=rcClient.top;
	// 	rcToolBar.bottom=rcClient.top+25;
	// 	rcToolBar.left=7;
	// 	rcToolBar.right=rcClient.right;
	// 	CBrush brush(RGB(255,255,255));
	// 	CBrush* pOldBrush=pDC->SelectObject(&brush);
	// 	pDC->Rectangle(&rcToolBar);	pDC->Draw3dRect(&rcToolBar,RGB(255,255,255),RGB(255,255,255));
	// 	pDC->SelectObject(pOldBrush);

	//////////////////////////////////////////////////////////////////////////
	DWORD dwOnline = m_ClientUserManager.GetOnLineCount();
	TCHAR szTemp[256]={0};
	...
	CListKind * pListKind=m_pListServer->GetListKind();
	_snprintf(szRoomTitle,sizeof(szRoomTitle),TEXT("[ %s ] - [ %s ] （%s）"),pListKind->GetItemInfo()->szKindName,
		m_pListServer->GetItemInfo()->szServerName,szTemp/*m_ClientUserManager.GetOnLineCount()*/);

	//输出标题
	pDC->SetBkMode(TRANSPARENT);
	pDC->SetTextColor(RGB(64,64,64));
	pDC->SelectObject(CSkinResourceManager::GetDefaultFont());
	CRect rcTitle(10,0,rcVorSplitter.left-BUTTON_AREA_WIDTH-8,m_ImageInfoTable.nTBorder);
...
// 	CSkinAide::DrawEncircleFrame(pDC,rcChat,m_EncircleChat);
	...
	CRect rcUserListIni(0,0, 0, 0);
	CGameFrame::GetRectFromIni(szResPath("GameFrame.ini"), TEXT("Room"), TEXT("UserList"), rcUserListIni, rcUserListIni);
	rcUserListIni.bottom = rcClient.Height()-rcUserListIni.bottom;
	rcUserListIni.right = rcClient.Width()-rcUserListIni.right;
	rcUserListIni.left = rcClient.Width()-rcUserListIni.left;
	m_rcList=rcUserListIni;
	CSkinAide::DrawEncircleFrame(pDC,rcUserListIni,m_EncircleList);

	CRect rcChatIni(0,0, 0, 0);
	CGameFrame::GetRectFromIni(szResPath("GameFrame.ini"), TEXT("Room"), TEXT("Chat"), rcChatIni, rcChatIni);
	rcChatIni.bottom = rcClient.Height()-rcChatIni.bottom;
	rcChatIni.right = rcClient.Width()-rcChatIni.right;
	rcChatIni.left = rcClient.Width()-rcChatIni.left;
	rcChatIni.top = rcClient.Height() - rcChatIni.top;
	m_rcChat = rcChatIni;
	CSkinAide::DrawEncircleFrame(pDC,rcChatIni,m_EncircleChat);

	CPoint ptPropertyBarIni(0,0);
	CGameFrame::GetPointFromIni(szResPath("GameFrame.ini"), TEXT("Room"), TEXT("PropertyBar"), ptPropertyBarIni, ptPropertyBarIni);
	ptPropertyBarIni.x = rcClient.Width() - ptPropertyBarIni.x;
	ptPropertyBarIni.y = rcClient.Height() - ptPropertyBarIni.y;
	CImageHandle ImagePropertyBar(&m_ImagePropertyBar);
	m_ImagePropertyBar.BitBlt(pDC->GetSafeHdc(), ptPropertyBarIni.x, ptPropertyBarIni.y);
	//绘画列表
// 	CRect rcList;
...
void CRoomViewItem::RectifyControl(int nWidth, int nHeight)
{
	...
	//竖向范围
	INT nVorMaxPos=nWidth-380-60+34;
	...
	if (m_bHideUserList==false)
	{
		if (m_bRectifyVor==false)
		{
			...
			rcVorSplitter.left=nWidth-380;
			rcVorSplitter.right=rcVorSplitter.left+nSplitterWidth+115;
		}
	}
	else
	{
	...
	if ((nWidth!=0)&&(nHeight!=0))
	{
		...
		if (!m_bHideUserList)
		{
			...
			{
				rcVorSplitter.right=nVorMaxPos+115;
	...
	//m_BrowerAD.BringWindowToTop();
	//广告区域

	CRect rcRoomTableFrameIni(0,0, 0, 0);
	CGameFrame::GetRectFromIni(szResPath("GameFrame.ini"), TEXT("Room"), TEXT("RoomTableFrame"), rcRoomTableFrameIni, rcRoomTableFrameIni);
	rcRoomTableFrameIni.bottom = nHeight-rcRoomTableFrameIni.bottom;
	rcRoomTableFrameIni.right = nWidth-rcRoomTableFrameIni.right;
	//rcRoomTableFrameIni.left = nWidth-rcRoomTableFrameIni.left;

	//桌子区域
	DeferWindowPos(hDwp,m_TableFrame,NULL,rcRoomTableFrameIni.left, rcRoomTableFrameIni.top, rcRoomTableFrameIni.Width(), rcRoomTableFrameIni.Height(),uFlags);

	SkinSB_Init(m_TableFrame,CGameFrame::m_bmScroll);
	//显示区域
	if(!m_bHideUserList)
	{
		//列表区域
		CRect rcUserListViewIni(0,0, 0, 0);
		CGameFrame::GetRectFromIni(szResPath("GameFrame.ini"), TEXT("Room"), TEXT("UserListView"), rcUserListViewIni, rcUserListViewIni);
		rcUserListViewIni.bottom = nHeight-rcUserListViewIni.bottom;
		rcUserListViewIni.right = nWidth-rcUserListViewIni.right;
		rcUserListViewIni.left = nWidth-rcUserListViewIni.left;
		DeferWindowPos(hDwp,m_UserListView,NULL,rcUserListViewIni.left, rcUserListViewIni.top, rcUserListViewIni.Width(), rcUserListViewIni.Height(),uFlags);
		//聊天区域
		CRect rcChatViewIni(0,0, 0, 0);
		CGameFrame::GetRectFromIni(szResPath("GameFrame.ini"), TEXT("Room"), TEXT("ChatView"), rcChatViewIni, rcChatViewIni);
		rcChatViewIni.bottom = nHeight-rcChatViewIni.bottom;
		rcChatViewIni.right = nWidth-rcChatViewIni.right;
		rcChatViewIni.left = nWidth-rcChatViewIni.left;
		rcChatViewIni.top = nHeight - rcChatViewIni.top;
		DeferWindowPos(hDwp,m_ChatMessage,NULL,rcChatViewIni.left, rcChatViewIni.top, rcChatViewIni.Width(), rcChatViewIni.Height(),uFlags);
	}

	//控制区域
	CPoint ptQuitRoomIni;
	CGameFrame::GetPointFromIni(szResPath("GameFrame.ini"), TEXT("Room"), TEXT("QuitRoom"), ptQuitRoomIni, ptQuitRoomIni);
	ptQuitRoomIni.x = nWidth-ptQuitRoomIni.x;
	
	CPoint ptAutoSitIni;
	CGameFrame::GetPointFromIni(szResPath("GameFrame.ini"), TEXT("Room"), TEXT("AutoSit"), ptAutoSitIni, ptAutoSitIni);
	ptAutoSitIni.x = nWidth-ptAutoSitIni.x;

	CPoint ptRoomSetIni;
	CGameFrame::GetPointFromIni(szResPath("GameFrame.ini"), TEXT("Room"), TEXT("RoomSet"), ptRoomSetIni, ptRoomSetIni);
	ptRoomSetIni.x = nWidth-ptRoomSetIni.x;

	m_btQuitRoom.SetWindowText("退出房间");
	m_btRoomSet.SetWindowText("设置");
	m_btAutoSit.SetWindowText("查找座位");
	DeferWindowPos(hDwp, m_btQuitRoom, NULL, ptQuitRoomIni.x, ptQuitRoomIni.y, 0,0, uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp, m_btRoomSet, NULL, ptRoomSetIni.x, ptRoomSetIni.y, 0,0, uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp, m_btAutoSit, NULL, ptAutoSitIni.x, ptAutoSitIni.y, 0,0, uFlags|SWP_NOSIZE);
	...
	//m_ChatMessageBig.ShowWindow((!m_bHideUserList)?SW_SHOW:SW_HIDE);
	...
	m_btBank.ShowWindow(0);
	if(m_wGameGenre!=GAME_GENRE_GOLD)
		m_btBank.ShowWindow(SW_HIDE);
	m_BrowerAD.ShowWindow(SW_HIDE);
	m_PropertyBar.ShowWindow(SW_HIDE);
	m_btFindUser.ShowWindow(SW_HIDE);
	m_btBindPc.ShowWindow(SW_HIDE);

	//////////////////////////////////////////////////////////////////////////
	//MODIF  去掉聊天窗口的四个按钮
	m_btExpression.ShowWindow(SW_HIDE);m_btColorSet.ShowWindow(SW_HIDE);m_btCleanScreen.ShowWindow(SW_HIDE);m_btBugle.ShowWindow(SW_HIDE);
	//////////////////////////////////////////////////////////////////////////
...
//	pUserInfoHead->cbAndroid=pUserData->cbAndroid;
...
bool CRoomViewItem::OnSocketMainUser(CMD_Command Command, void * pData, WORD wDataSize)
{
	...
	case SUB_GR_CALLUSER:			//呼叫用户
		{
			return OnSocketCallUser(Command,pData,wDataSize);
		}
	...
	case SUB_GR_CONFIG_FINISH:	//配置完成
		{
			...
			m_UserListView.m_GameRankManager.CreateInstance();
			//游戏等级
...
bool CRoomViewItem::OnSocketMainGameFrame(CMD_Command Command, void * pData, WORD wDataSize)
{
	...
	else if( Command.wSubCmdID == SUB_GF_FLOWER )
	{
		...
		if(pGiftNotify->cbSendLocation==LOCATION_PLAZA_ROOM)
		{
			...
			strGiftMsg.Format(TEXT("[%s]"),pSendUserData->szGroupName);
			...
			strGiftMsg.Format(TEXT("[%s]"),pRcvUserData->szGroupName);
			m_MessageProxyHelper->InsertShtickString(strGiftMsg,RGB(0,0,255),strGiftMsg.GetLength(),false);
			...
			strGiftMsg.Format(TEXT("，共%d%s%s[ %s ]"), pGiftNotify->wFlowerCount, g_FlowerDescribe[nFlowerID].szQuantifier2,
				g_FlowerDescribe[nFlowerID].szResult, pRcvUserData->szGroupName);
			m_MessageProxyHelper->InsertShtickString(strGiftMsg,RGB(255,0,255), strGiftMsg.GetLength(), false);	
...
//	UserData.cbAndroid = pUserInfoHead->cbAndroid;
		...
		case DTP_USER_ACCOUNTS:		//用户帐户
			{
				...
				{
					lstrcpyn(UserData.szGroupName,TEXT("游戏用户"),CountArray(UserData.szGroupName));
				}
				break;
			}
	...
	if (pIUserItem!=NULL)
	{
		...
		if ((m_ServiceStatus==ServiceStatus_Serviceing))
		{
			...
			{
				TCHAR szMessage[256]=TEXT("");
				_snprintf(szMessage,sizeof(szMessage),TEXT("[ %s ] 进来了"),UserData.szGroupName);
				//m_MessageProxyHelper->InsertSystemString(szMessage,0);
				m_MessageProxyHelper->InsertShtickString(szMessage,RGB(115,115,115), sizeof(szMessage), true);
			}
...
bool CRoomViewItem::OnSocketSubStatus(CMD_Command Command, void * pData, WORD wDataSize)
{
	...
	if (cbNowStatus==US_NULL)
	{
		...
		TCHAR szMessage[256]=TEXT("");
		_snprintf(szMessage,sizeof(szMessage),TEXT("[ %s ] 离开了"),pUserData->szGroupName);
		m_MessageProxyHelper->InsertShtickString(szMessage,RGB(115,115,115), sizeof(szMessage), true);
		//删除用户
	...
	if (pIUserItem==m_pMeUserItem)
	{
		...
		if ((wNowTableID!=INVALID_TABLE)&&((wNowTableID!=wLastTableID)||(wNowChairID!=wLastChairID)))
		{
			...
			if ((m_GameProcessInfo.hProcess!=NULL)&&(WaitForSingleObject(m_GameProcessInfo.hProcess,0)==WAIT_TIMEOUT))
			{
				if (m_pShareMemory->hWndGameFrame!=NULL)
				{
					m_TableFrame.ShowStartBtn(INVALID_TABLE, INVALID_CHAIR, false);
					StartGameClient();
					return true;
				}
			}
			m_TableFrame.ShowStartBtn(wNowTableID, wNowChairID, true);
		}
	}
...
bool CRoomViewItem::OnSocketSubChat(CMD_Command Command, void * pData, WORD wDataSize)
{
	...
	if (pUserChat->dwTargetUserID!=0L)
	{
		...
		m_MessageProxyHelper->InsertUserChat(pSendUserData->szGroupName,pRecvUserData->szGroupName,pUserChat->szChatMessage,pUserChat->crFontColor,MS_NORMAL);
	}
	else m_MessageProxyHelper->InsertUserChat(pSendUserData->szGroupName,pUserChat->szChatMessage,pUserChat->crFontColor,MS_NORMAL);
...
bool CRoomViewItem::OnSocketSubWisper(CMD_Command Command, void * pData, WORD wDataSize)
{
	...
	if (pWisper->dwSendUserID==m_pMeUserItem->GetUserID())
	{
		...
		if (pShortMessageWnd!=NULL) pShortMessageWnd->OnRecvMessage(pUserDataSend->szGroupName,pWisper->szChatMessage,pWisper->crFontColor,true);
	}
	else	//其他游戏者发送信息
	{
...
bool CRoomViewItem::OnSocketSubUserInvite(CMD_Command Command, void * pData, WORD wDataSize)
{
	...
	//提示消息
	TCHAR szMessage[256]=TEXT("");
	_snprintf(szMessage,sizeof(szMessage),TEXT("[ %s ] 邀请你加入第 [ %ld ] 游戏桌进行游戏，是否同意？"),pUserData->szGroupName,pUserInvite->wTableID+1);
...
bool CRoomViewItem::SetChatObject(IUserItem * pIUserItem)
{
	if (pIUserItem!=NULL)
	{
		...
		lstrcpyn(m_szChatName,pUserData->szGroupName,CountArray(m_szChatName));
...
void CRoomViewItem::ShowBankDlg()
{
	//类型判断	
	if (m_wGameGenre==GAME_GENRE_SCORE)
	{
		ShowInformation(TEXT("积分房间不可以使用此功能！"),0,MB_ICONINFORMATION);
		return ;
	}
	...
	if (m_wGameGenre==GAME_GENRE_EDUCATE)
	{
		ShowInformation(TEXT("练习房间不可以使用此功能！"),0,MB_ICONINFORMATION);
		return ;
	}
	//获取用户
	tagGlobalUserData &GlobalUserData = g_GlobalUnits.GetGolbalUserData();
...
//绑定机器
void CRoomViewItem::OnBindPc()
{
	::SendMessage(AfxGetMainWnd()->m_hWnd, WM_BIND_PC, 0, 0);
}

bool CRoomViewItem::ShowPlayerInfo(IUserItem * pIUserItem, CPoint Point)
{
	//获取用户
	ASSERT(pIUserItem!=NULL);
	if (pIUserItem==NULL) return false;

	if (m_pDlgPlayerInfo==NULL) 
	{
		m_pDlgPlayerInfo=new CDlgPlayerInfo(this,pIUserItem->GetUserData());
	}
	if (m_pDlgPlayerInfo->GetSafeHwnd()==NULL)
	{
		m_pDlgPlayerInfo->Create(IDD_PLAYER_INFO,this);
		//m_pDlgPlayerInfo->m_UserList.SetColumnDescribe(m_ListColumnInfo.ColumnItem,m_ListColumnInfo.wColumnCount);
	}
	m_pDlgPlayerInfo->UpdataUserData(pIUserItem->GetUserData());
	m_pDlgPlayerInfo->UpdataUserInfo();
	CRect rcDlg;
	m_pDlgPlayerInfo->GetWindowRect(&rcDlg);
	CRect rcUserList;
	m_UserListView.GetWindowRect(&rcUserList);
// 	rcDlg.top = Point.y;
// 	rcDlg.right=rcUserList.left;
// 	rcDlg.left=rcUserList.left-300;
// 	rcDlg.bottom = Point.y+400;
	m_pDlgPlayerInfo->SetWindowPos(NULL, rcUserList.left-rcDlg.Width(), Point.y, 0,0,SWP_NOSIZE);

	m_pDlgPlayerInfo->ShowWindow(SW_SHOW);


	return true;
}

bool CRoomViewItem::OnSocketCallUser(CMD_Command Command, void * pData, WORD wDataSize)
{
	//效验参数
	ASSERT(Command.wMainCmdID==MDM_GR_USER);
	ASSERT(Command.wSubCmdID==SUB_GR_CALLUSER);
	...
	//用户过虑
	if (pSendUserData->cbCompanion==enCompanion_Detest) return true;
	if (g_GlobalOption.m_InviteMode==enCallMode_None) return true;
	if ((g_GlobalOption.m_InviteMode==enCallMode_Friend)&&(pSendUserData->cbCompanion!=enCompanion_Friend)) return true;
	...
	if (pUserChat->dwTargetUserID!=0L)
	{
		...
		//m_MessageProxyHelper->InsertUserChat(pSendUserData->szGroupName,pRecvUserData->szGroupName,pUserChat->szChatMessage,pUserChat->crFontColor,MS_NORMAL);
		TCHAR szMsg[256];
		_snprintf(szMsg, CountArray(szMsg), TEXT("玩家[%s],呼叫你！"), pSendUserData->szName);
		m_MessageProxyHelper->InsertSystemString(szMsg, 0);

		//播放声音
		HINSTANCE hInstance = AfxGetInstanceHandle();
		HRSRC hResour=FindResource(hInstance,TEXT("CALLUSER"),TEXT("WAVE"));
		if (hResour!=NULL)
		{
			HGLOBAL hGlobal=LoadResource(hInstance,hResour);
			if (hGlobal!=NULL) 
...
void __cdecl CRoomViewItem::OnStartGame( WORD wTableID, WORD wChairID, bool bMirror )
{
	//判断状态
// 	if (m_ServiceStatus==ServiceStatus_NetShutDown)
// 	{
...
bool CRoomViewItem::FindGameTable2( tagFindTable & FindInfo )
{
	//变量定义
	WORD wNullCount=0;
	ITableView * pTableView=NULL;
