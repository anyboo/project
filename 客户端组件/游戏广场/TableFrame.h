...
{
	//初始化函数
	virtual void __cdecl InitTableView(WORD wTableID, WORD wChairCount,WORD wKindId, ITableFrameView * pITableFrameView)=NULL;
	//设置用户信息
	...
	//绘画函数
	virtual void __cdecl DrawTable(CDC * pDC, bool bHideInfo, CTableResource * pTableResource, bool bShowUser,int X,int Y)=NULL;
};
...
interface ITableFrameSink : public IUnknownEx
{
	...
	//点击开始 
	virtual void __cdecl OnStartGame(WORD wTableID, WORD wChairID, bool bMirror)=NULL;
};
...
public:
    ...
    CPoint								m_ptPalyer[MAX_CHAIR];							//锁位置
	//颜色参数
public:
...
public:
	...
	CSkinImage							m_ImageJinRu;						//百人房间内进入位图
	CSkinImage							m_ImageHao;						    //其他房间内进入位图
	CSkinImage							m_ImagePlay;						//游戏位图
	CSkinImage							m_ImageReady;						//准备位图
	CSkinImage							m_ImageTable;						//桌子位图
	CSkinImage							m_ImagePeople;						//用户位图
    CPngImage						    m_ImageTableNormal;						//桌子图片
	CPngImage						    m_ImageTablePlay;						//桌子图片
	CSkinImage							m_ImageStart;						//游戏开始
	CSkinImage							m_ImageFree;						//游戏空闲
	CSkinImage							m_ImageNumBack;						//桌子编号

	
                          
// 	CPngImage						    m_ImageGrilPlayr[5][17];						//桌子图片
// 	CPngImage						    m_ImageBoyPlayr[5][17];						//桌子图片
// 
// 	CPngImage						    m_ImageGrilPlayr3[5][17];						//桌子图片
// 	CPngImage						    m_ImageBoyPlayr3[5][17];

	CPngImage							m_ImageBoyPlayer1[5][17];
	CPngImage							m_ImageBoyPlayer2[5][17];

	CPngImage							m_ImageGrilPlayer1[5][17];
	CPngImage							m_ImageGrilPlayer2[5][17];
...
protected:
	...
	bool LoadFromFilesSize(bool bHideInfo,LPCTSTR pszGameName, WORD wChairCount, WORD wKindID);
	//卸载资源
...
// 
// 	CPngImage						    m_ImageGrilPlayr[5];						//桌子图片
// 	CPngImage						    m_ImageBoyPlayr[5];						//桌子图片

	CSkinImage							m_ImageViewCenter;					//背景资源
	CImageList						m_MemberImage;							// 会员图标
...
public:
	//初始化函数
	virtual void __cdecl InitTableView(WORD wTableID, WORD wChairCount, WORD wKindId, ITableFrameView * pITableFrameView);
	//设置用户信息
...
public:
	//绘画函数
	virtual void __cdecl DrawTable(CDC * pDC, bool bHideInfo, CTableResource * pTableResource, bool bShowUser,int X,int Y);
};
...
public:
	...
	CToolTipCtrl						m_pToolTipCtrl;						//玩家信息提示
	CGdiButton							m_btStart;							//开始按钮		
...
public:
	...
	//显示按钮 
	virtual bool __cdecl ShowStartBtn(WORD wTableID,WORD wChairId, bool bShowStart);
...
protected:
	...
	//提示文字
	LPCTSTR GetTipsText(IUserItem * pIUserItem);
