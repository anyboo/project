...
CGlobalOption::CGlobalOption()
{
	...
	m_crMsgSysString=RGB(200,0,0);
	m_crServerListTX=RGB(0,0,0);
	m_crServerListBK=RGB(200,200,200);

	m_crOwnColor=RGB(255,0,0);
	m_crFriendColor=RGB(255,0,128);
	//代理配置
...
void CGlobalOption::LoadOptionParameter()
{
	...
	m_CallMode = (enCallMode)pApp->GetProfileInt(REG_GLOBAL_OPTIONS,TEXT("CallMode"),m_CallMode);
	//保险箱配置
	m_bCloseQuery=pApp->GetProfileInt(REG_GLOBAL_OPTIONS,TEXT("QueryClose"),m_bCloseQuery)?true:false;
	m_BankCloseTip=pApp->GetProfileInt(REG_GLOBAL_OPTIONS,TEXT("BankCloseTip"),m_BankCloseTip)?true:false;
	m_BankCloseMode=(enBankRule)pApp->GetProfileInt(REG_GLOBAL_OPTIONS,TEXT("BankCloseMode"),m_BankCloseMode);
	m_BankCloseTime=(enBankRule)pApp->GetProfileInt(REG_GLOBAL_OPTIONS,TEXT("BankCloseTime"),m_BankCloseTime);
	m_crOwnColor=(enBankRule)pApp->GetProfileInt(REG_GLOBAL_OPTIONS,TEXT("OwnColor"),m_crOwnColor);
	m_crFriendColor=(enBankRule)pApp->GetProfileInt(REG_GLOBAL_OPTIONS,TEXT("FriendColor"),m_crFriendColor);

	//坐桌密码
	m_bPassword = pApp->GetProfileInt(REG_GLOBAL_OPTIONS,TEXT("SitHavePass"),m_bPassword)?true:false;
	CopyMemory(m_szPassword,pApp->GetProfileString(REG_GLOBAL_OPTIONS,TEXT("SitPassword"),TEXT("")),CountArray(m_szPassword));

		//限制IP 修改
	m_bSameIp = pApp->GetProfileInt(REG_GLOBAL_OPTIONS,TEXT("SameIp"),m_bSameIp)?true:false;
	m_wSameIpNum=(enBankRule)pApp->GetProfileInt(REG_GLOBAL_OPTIONS,TEXT("SameIpNum"),m_wSameIpNum);
...
void CGlobalOption::SaveOptionParameter()
{
	...
	pApp->WriteProfileInt(REG_GLOBAL_OPTIONS,TEXT("CallMode"),m_CallMode);
	...
	//保险箱配置 修改
	pApp->WriteProfileInt(REG_GLOBAL_OPTIONS,TEXT("QueryClose"),m_bCloseQuery);
	pApp->WriteProfileInt(REG_GLOBAL_OPTIONS,TEXT("BankCloseTip"),m_BankCloseTip);
	pApp->WriteProfileInt(REG_GLOBAL_OPTIONS,TEXT("BankCloseMode"),m_BankCloseMode);
	pApp->WriteProfileInt(REG_GLOBAL_OPTIONS,TEXT("BankCloseTime"),m_BankCloseTime);
	pApp->WriteProfileInt(REG_GLOBAL_OPTIONS,TEXT("OwnColor"),m_crOwnColor);
	pApp->WriteProfileInt(REG_GLOBAL_OPTIONS,TEXT("FriendColor"),m_crFriendColor);

	//坐桌密码
	pApp->WriteProfileInt(REG_GLOBAL_OPTIONS,TEXT("SameIp"),m_bSameIp);
	pApp->WriteProfileInt(REG_GLOBAL_OPTIONS,TEXT("SameIpNum"),m_wSameIpNum);

	//相同IP
	pApp->WriteProfileInt(REG_GLOBAL_OPTIONS,TEXT("SitHavePass"),m_bPassword);
	pApp->WriteProfileString(REG_GLOBAL_OPTIONS,TEXT("SitPassword"),m_szPassword);
	//游戏配置
	CGameOption * pGameOption=NULL;
	for (INT_PTR i=0;i<m_GameOptionArray.GetCount();i++)
	{
