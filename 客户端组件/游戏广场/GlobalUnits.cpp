...
bool CGlobalUnits::InitGlobalUnits()
{
	...
	//加载组件
	if (m_UserFaceRes.CreateInstance()==false) 
	{
		
	return false;
	}
	if (m_DownLoadService.CreateInstance()==false) 	{
		
	return false;
	}
	if (m_CompanionManager.CreateInstance()==false) 	{
...
bool CGlobalAttemper::DownLoadClient(LPCTSTR pszKindName, WORD wKindID, bool bDisplay)
{
	...
	_snprintf(DownLoadRequest.szDownLoadUrl,sizeof(DownLoadRequest.szDownLoadUrl),TEXT("http://www.7x78.com/Download.asp?KindID=%ld&LocalVersion=0&PlazaVersion=%ld"),wKindID,VER_PLAZA_FRAME);
