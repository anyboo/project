...
public:
	...
	WORD								m_wKindID;							//类型标识
	int                                 Level_Count;						//画钻石的控制
    LONGLONG                           pszGameRank11;
	BYTE								m_cbMaster;							//是否是管理员
	//资源变量
protected:
	UINT								m_uImageSpace;						//位图间隔
	BYTE								m_cbAscendSort;						//升序标志
	static CImageList					m_StatusImage;						//状态位图
	static CImageList					m_StatusImage1;						//状态位图
	static CImageList					m_GenderImage;						//男女表示
	static CImageList					m_MemberImage;						//会员标识
