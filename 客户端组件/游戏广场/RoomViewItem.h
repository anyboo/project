...
protected:
	...
	//聊天控件
public:
	CMyComboBox						m_ChatInput;						//聊天输入
	CMyComboBox						m_ChatObject;						//聊天对象
...
public:
	...
	CSkinButton						m_btRoomSet;						//设置按钮
     HBITMAP	                    m_hBmpScrollBar	;
	CSkinButton						m_btBindPc;							//绑定机器
...
public:
	...
	CWebBrowser						m_BrowerAD1;							//浏览窗口
	CTableFrame						m_TableFrame;						//桌子框架
	CUserListView					m_UserListView;						//用户列表
	CDlgPlayerInfo					*m_pDlgPlayerInfo;					//玩家信息
	CPtrArray						m_ShortMessage;						//信息窗口
	CFindUserDlg					* m_pFindUserDlg;					//查
	CBankStorage                    *m_pBankDlg;                       //银行按钮       
	CDlgFindTable					*m_pFindTable;						//查找桌子
...
public:
	CString                          ClentIP;                          //客户端的IP地址
    DWORD                            GameID;                            //本家用户的GameID;
	//管理控件
public:
...
protected:
	...
	CSkinImage						m_ImagePropertyBar;					//道具栏
...
public:
	...
	//点击开始 
	virtual void __cdecl OnStartGame(WORD wTableID, WORD wChairID, bool bMirror);
	//用户接口
public:
	//用户激活
...
protected:
	...
	//呼叫用户
	bool OnSocketCallUser(CMD_Command Command, void * pBuffer, WORD wDataSize);
	//用户私语
...
public:
	...
	//玩家资料
	bool ShowPlayerInfo(IUserItem * pIUserItem, CPoint Point);
...
public:
	...
	//查找桌子
	bool FindGameTable2(tagFindTable & FindInfo);
...
protected:
	...
	//设置按钮
	afx_msg void OnBnClickedRoomSet();
	//绑定机器
	afx_msg void OnBindPc();
