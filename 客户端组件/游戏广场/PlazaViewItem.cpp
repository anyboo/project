...
CPlazaViewItem::CPlazaViewItem() : CDialog(IDD_GAME_PLAZA)
{
	...
	m_pHtmlBrower	= NULL;
    m_enBindAction=enBindAction_NULL;
	return ;
}
...
BOOL CPlazaViewItem::OnInitDialog()
{
	...
	//浏览野蛮
	m_pHtmlBrower->Navigate("http://www.7x78.com/mini.asp");
	WebBrowse("www.7x78.com/mini.asp",true);
	//创建按钮
...
bool __cdecl CPlazaViewItem::OnEventTCPSocketLink(WORD wSocketID, INT nErrorCode)
{
	...
	if (nErrorCode!=0)
	{
    	...
    	//绑定判断
	if (enBindAction_Bind==m_enBindAction)
	{
		//获取玩家
		...
		TCHAR szPassword[33];
		CMD5Encrypt::EncryptData(pGameFrame->m_DlgLockMachine.m_strPassword,szPassword);
		...
		//发送数据
		m_ClientSocket->SendData(MDM_GP_USER,SUB_GP_SAFE_BIND,&SafeBind,sizeof(SafeBind));
         
		//设置变量
		m_enBindAction=enBindAction_NULL;
	}
	else if (enBindAction_UnBind==m_enBindAction)
	{
		//获取玩家
		tagGlobalUserData &GlobalUserData=g_GlobalUnits.GetGolbalUserData();
		...
		//发送数据
		m_ClientSocket->SendData(MDM_GP_USER,SUB_GP_SAFE_UNBIND,&SafUnbind,sizeof(SafUnbind));
	...
	CDlgMoneyManager &DlgMoneyManager = pGameFrame->m_DlgBankManager.m_DlgMoneyManager;
	if ( DlgMoneyManager.m_hWnd != NULL && DlgMoneyManager.IsWindowVisible() )
	{
		DlgMoneyManager.SendData();
	}
	CDlgMoneyGive &DlgMoneyGive = pGameFrame->m_DlgBankManager.m_DlgMoneyGive;
	if ( DlgMoneyGive.m_hWnd != NULL && DlgMoneyGive.IsWindowVisible() )
	{
		DlgMoneyGive.SendData();
	}

	CDlgMoneyPass &DlgMoneyPass = pGameFrame->m_DlgBankManager.m_DlgMoneyPass;
	if ( DlgMoneyPass.m_hWnd != NULL && DlgMoneyPass.IsWindowVisible() )
	{
		DlgMoneyPass.SendData();
	}

	CDlgMoneyRecord &DlgMoneyRecord = pGameFrame->m_DlgBankManager.m_DlgMoneyRecord;
	if ( DlgMoneyRecord.m_hWnd != NULL && DlgMoneyRecord.IsWindowVisible() )
	{
		DlgMoneyRecord.SendData();
	}

	CDlgUserLock &DlgUserLock = pGameFrame->m_DlgUserManager.m_DlgUserLock;
	if ( DlgUserLock.m_hWnd != NULL && DlgUserLock.IsWindowVisible() )
	{
		DlgUserLock.SendData();
	}
    
	CDlgUserSample &DlgUserSample = pGameFrame->m_DlgUserManager.m_DlgUserSample;
	if ( DlgUserSample.m_hWnd != NULL && DlgUserSample.IsWindowVisible() )
	{
		DlgUserSample.SendData();
	}

	CDlgUserPass &DlgUserPass = pGameFrame->m_DlgUserManager.m_DlgUserPass;
	if ( DlgUserPass.m_hWnd != NULL && DlgUserPass.IsWindowVisible() )
	{
		DlgUserPass.SendData();
	}

	CDlgUserXX &DlgUserXX = pGameFrame->m_DlgUserManager.m_DlgUserXX;
	if ( DlgUserXX.m_hWnd != NULL && DlgUserXX.IsWindowVisible() )
	{
		DlgUserXX.SendData();
	}
...
bool __cdecl CPlazaViewItem::OnEventTCPSocketShut(WORD wSocketID, BYTE cbShutReason)
{
	...
	CGameFrame *pGameFrame = (CGameFrame *)AfxGetMainWnd() ;
//	CDlgMoneyManager &DlgMoneyManager = pGameFrame->m_DlgMoneyManager;
//	if ( DlgMoneyManager.m_hWnd != NULL ) DlgMoneyManager.SetStatus(enOperateMoneyStatus_NULL);

	CDlgMoneyManager &DlgMoneyManager = pGameFrame->m_DlgBankManager.m_DlgMoneyManager;
	if ( DlgMoneyManager.m_hWnd != NULL && DlgMoneyManager.IsWindowVisible() )
	{
		DlgMoneyManager.SetStatus(enOperateMoneyStatus_NULL);
	}

	CDlgMoneyGive &DlgMoneyGive = pGameFrame->m_DlgBankManager.m_DlgMoneyGive;
	if ( DlgMoneyGive.m_hWnd != NULL && DlgMoneyGive.IsWindowVisible() )
	{
		DlgMoneyGive.SetStatus(enOperateMoneyGiveStatus_NULL);
	}
   //设置变量
	m_enBindAction=enBindAction_NULL;
	return true;
	...
	case SUB_GP_LOGON_SUCCESS:		//登录成功
		{
			...
			//UserData.JiQi=pLogonSuccess->JiQi;
			UserData.JiQi=pLogonSuccess->JiQi;

			//设置按钮
			UINT uBtResurce=UserData.JiQi?IDB_UNLOCK:IDB_LOCK;
			UINT uBtResurce1=UserData.cbMember?IDB_HONGZUAN:IDB_UNZUAN;
			CGameFrame *pGameFrame=(CGameFrame*)AfxGetMainWnd();
			//pGameFrame->m_btLock.SetButtonImage(uBtResurce,AfxGetInstanceHandle(),false);
			//pGameFrame->m_btHongzuan.SetButtonImage(uBtResurce1,AfxGetInstanceHandle(),false);

			//会员信息
			//UINT uBtResurce=UserData.cbMember?IDB_UNZUAN:IDB_HONGZUAN;
			//CGameFrame *pGameFrame=(CGameFrame*)AfxGetMainWnd();
			//pGameFrame->m_btHongzuan.SetButtonImage(uBtResurce,AfxGetInstanceHandle(),false);
				...
				case DTP_BANK_PASS:
					{
						...
						if (pDataBuffer!=NULL) 
						{
							CopyMemory(UserData.szBankPass,pDataBuffer,DataDescribe.wDataSize);
							UserData.szBankPass[CountArray(UserData.szBankPass)-1]=0;
	...
	case SUB_GP_LOGON_FINISH:		//登录完成
		{
			...
			//更新登录时间
			((CGameFrame*)AfxGetMainWnd())->m_tmLogonTime = CTime::GetCurrentTime();
			...
			m_pHtmlBrower->Navigate("http://www.7x78.com/mini.asp");
			WebBrowse("http://www.7x78.com/mini.asp",true);
			if(GlobalUserData.JiQi == false)
				m_SafeTip.DoModal();
	...
	case SUB_GP_LIST_SERVER:		//服务器房间
		{
			...
			//就是这里的排序
            SortCardList(pGameServer,wItemCount);
	...
	case SUB_GP_LIST_FINISH:		//列表发送完成
		{
			...
			((CGameFrame*)AfxGetMainWnd())->OnLineCount=dwAllOnLineCount;
             ((CGameFrame*)AfxGetMainWnd())->Invalidate(FALSE);
			return true;
		}
...
void CPlazaViewItem::SortCardList(tagGameServer cbCardData[], BYTE cbCardCount)
{
//转换数值
	tagGameServer cbLogicValue[100];
	for (BYTE i=0;i<cbCardCount;i++) cbLogicValue[i]=cbCardData[i];	

	//排序操作
	bool bSorted=true;
	BYTE bLast=cbCardCount-1;

	tagGameServer cbTempData;
	do
	{
		bSorted=true;
		for (BYTE i=0;i<bLast;i++)
		{
			if ((cbLogicValue[i].wServerID<cbLogicValue[i+1].wServerID)||
				((cbLogicValue[i].wServerID==cbLogicValue[i+1].wServerID)&&(cbCardData[i].wServerID<cbCardData[i+1].wServerID)))
			  {
				//交换位置
...
bool CPlazaViewItem::OnSocketMainUser(CMD_Command Command, void * pBuffer, WORD wDataSize)
{
	...
	switch(Command.wSubCmdID)
	{
		case SUB_GP_SAFE_BIND:					//绑定机器
		{
			CMD_GP_BindResult *pResult=(CMD_GP_BindResult *)pBuffer;

		
			if (!pResult->cbSuccess)
			{
				//显示消息
				ShowMessageBox(TEXT("恭喜你绑定成功！"),MB_ICONINFORMATION);

				//获取玩家
				tagGlobalUserData &GlobalUserData=g_GlobalUnits.GetGolbalUserData();
                
				//设置标识
				GlobalUserData.JiQi=1;

				CGameFrame *pGameFrame = (CGameFrame *)AfxGetMainWnd() ;
				CDlgUserLock &DlgUserInfoLock = pGameFrame->m_DlgUserManager.m_DlgUserLock;
				if ( DlgUserInfoLock.m_hWnd != NULL && DlgUserInfoLock.IsWindowVisible() )
					DlgUserInfoLock.InitRoomName();
			}
			else
			{
				//显示消息
				ShowMessageBox(TEXT("输入密码有误，绑定失败！"),MB_ICONINFORMATION);
			}
	...
	case SUB_GP_SAFE_UNBIND:				//绑定机器
		{
			...
			{
				//显示消息
				ShowMessageBox(TEXT("恭喜你解除绑定成功！"),MB_ICONINFORMATION);

				//获取玩家
				tagGlobalUserData &GlobalUserData=g_GlobalUnits.GetGolbalUserData();

				//设置标识
				GlobalUserData.JiQi=0;
				CGameFrame *pGameFrame = (CGameFrame *)AfxGetMainWnd() ;
				CDlgUserLock &DlgUserInfoLock = pGameFrame->m_DlgUserManager.m_DlgUserLock;
				if ( DlgUserInfoLock.m_hWnd != NULL && DlgUserInfoLock.IsWindowVisible() )
					DlgUserInfoLock.InitRoomName();
			...
			{
				//显示消息
				ShowMessageBox(TEXT("输入密码有误，解除绑定失败！"),MB_ICONINFORMATION);
			}
	...
	case SUB_GP_MODIFY_INDIVIDUAL_RESULT:	//修改结果
		{
			...
			if ( DlgUserInfoLock.m_hWnd != NULL && DlgUserInfoLock.IsWindowVisible() )
			{
				if(ModifyIndividualResult.bOperateSuccess)
				{
					tagGlobalUserData &GlobalUserData = g_GlobalUnits.GetGolbalUserData();
					if(GlobalUserData.JiQi==0)
						GlobalUserData.JiQi=1;
					else
						GlobalUserData.JiQi=0;

					DlgUserInfoLock.InitRoomName();
					ShowMessageBox(TEXT("恭喜你操作成功！"),MB_ICONINFORMATION);
				}
				else
					ShowMessageBox(ModifyIndividualResult.szDescribeMsg,MB_ICONINFORMATION);
				
			}

			
			CDlgUserPass &DlgUserInfoPass = pGameFrame->m_DlgUserManager.m_DlgUserPass;

			if ( DlgUserInfoPass.m_hWnd != NULL && DlgUserInfoPass.IsWindowVisible() )
			{
				if(ModifyIndividualResult.bOperateSuccess)
				{
			...
			CDlgUserSample &DlgUserInfoSample = pGameFrame->m_DlgUserManager.m_DlgUserSample;

			if ( DlgUserInfoSample.m_hWnd != NULL && DlgUserInfoSample.IsWindowVisible() )
			{
				if(ModifyIndividualResult.bOperateSuccess)
				{
	...
	case SUB_GP_OPEN_MONEY:	//操作保险柜 2012-4-10
		{
			ASSERT(sizeof(CMD_GP_MoneyOperResult) == wDataSize);
			if ( sizeof(CMD_GP_MoneyOperResult) != wDataSize) return true;

			//显示消息
			CMD_GP_MoneyOperResult *pResult = (CMD_GP_MoneyOperResult*)pBuffer;
			ShowMessageBox(pResult->szDescribeMsg, MB_ICONINFORMATION);
				//设置界面
			CGameFrame *pGameFrame = (CGameFrame *)AfxGetMainWnd() ;
			CDlgMoneyManager &DlgMoneyManager = pGameFrame->m_DlgBankManager.m_DlgMoneyManager;
			if ( DlgMoneyManager.m_hWnd != NULL && DlgMoneyManager.IsWindowVisible() )
			{
				//关闭链接
				m_ClientSocket->CloseSocket();
				if(pResult->bOperateSuccess)
				{
					DlgMoneyManager.OnBnClickedRefresh();
				}
				else
					DlgMoneyManager.UpdateControls();
		...
		case SUB_GP_GIVE_MONEY:	//操作保险柜 2012-4-10
		{
			ASSERT(sizeof(CMD_GP_MoneyGiveResult) == wDataSize);
			if ( sizeof(CMD_GP_MoneyGiveResult) != wDataSize) return true;

			//显示消息
			CMD_GP_MoneyGiveResult *pResult = (CMD_GP_MoneyGiveResult*)pBuffer;
			
			//设置界面
			CGameFrame *pGameFrame = (CGameFrame *)AfxGetMainWnd() ;
			CDlgMoneyGive &DlgMoneyGive = pGameFrame->m_DlgBankManager.m_DlgMoneyGive;

			if(pResult->bQueryInfo == false)
			{
				ShowMessageBox(pResult->szDescribeMsg, MB_ICONINFORMATION);
			}
			else
			{
				if ( DlgMoneyGive.m_hWnd != NULL && DlgMoneyGive.IsWindowVisible() )
				{
					//关闭链接
					m_ClientSocket->CloseSocket();
					//DlgMoneyGive.SetMoney(pResult->lTotalScore,0);
					DlgMoneyGive.ShowOk(pResult->szTargeAccount, pResult->dwTargetUserId, pResult->lSwapScore);
					DlgMoneyGive.UpdateControls();
					return true;
				}
			}
			...
			if (DlgMoneyGive.m_hWnd != NULL && DlgMoneyGive.IsWindowVisible() )
			{
				...
				m_ClientSocket->CloseSocket();
				if(pResult->bOperateSuccess)		
					DlgMoneyGive.OnBnClickedRefresh();
				else
					DlgMoneyGive.UpdateControls();
				return true;
			}
	...
	case SUB_GP_REFRESH_MONEY:	//刷新保险柜 2012-4-10
		{
			ASSERT(sizeof(CMD_GP_MoneyRefreshResult) == wDataSize);
			if ( sizeof(CMD_GP_MoneyRefreshResult) != wDataSize) return true;

			//显示消息
			CMD_GP_MoneyRefreshResult *pResult = (CMD_GP_MoneyRefreshResult*)pBuffer;
			...
			if ( DlgMoneyManager.m_hWnd != NULL && DlgMoneyManager.IsWindowVisible() )
			{
				DlgMoneyManager.SetMoney(pResult->lTotalScore,pResult->lRoomScore);
				DlgMoneyManager.UpdateControls();
			}
	...
	case SUB_GP_EDIT_BANK_PASS:	//操作保险柜 2012-4-10
		{
			ASSERT(sizeof(CMD_GP_EditBankPassResult) == wDataSize);
			if ( sizeof(CMD_GP_EditBankPassResult) != wDataSize) return true;

			//显示消息
			CMD_GP_EditBankPassResult *pResult = (CMD_GP_EditBankPassResult*)pBuffer;
			ShowMessageBox(pResult->szDescribeMsg, MB_ICONINFORMATION);
	...
	case SUB_GP_GIVE_RECORD:	//操作保险柜 2012-4-10
		{
		//	ASSERT(sizeof(CMD_GP_EditBankPassResult) == wDataSize);
		//	if ( sizeof(CMD_GP_EditBankPassResult) != wDataSize) return true;

			//显示消息
			CMD_GP_BankRecordResult *pResult = (CMD_GP_BankRecordResult*)pBuffer;
			//ShowMessageBox(pResult->szDescribeMsg, MB_ICONINFORMATION);
			...
			if ( DlgMoneyRecord.m_hWnd != NULL && DlgMoneyRecord.IsWindowVisible() )
			{
				DlgMoneyRecord.m_nCount=pResult->nCount;
				memcpy(DlgMoneyRecord.GiveRecord,pResult->GiveRecord,pResult->nCount*sizeof(tagUserGiveRecord));
				DlgMoneyRecord.InitTable();
			}
...
void CPlazaViewItem::OnSize(UINT nType, int cx, int cy)
{
	...
	DeferWindowPos(hDwp,m_WEB_NEXT,NULL,15+rcButton.Width(),3,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_WEB_BRUSH,NULL,20+rcButton.Width()*2,3,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_WEB_STOP,NULL,25+rcButton.Width()*3,3,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_WEB_HOME,NULL,30+rcButton.Width()*4,3,0,0,uFlags|SWP_NOSIZE);

	EndDeferWindowPos(hDwp);

		m_WEB_BEFORE.ShowWindow(0);m_WEB_BRUSH.ShowWindow(0);
		m_WEB_HOME.ShowWindow(0);m_WEB_NEXT.ShowWindow(0);m_WEB_STOP.ShowWindow(0);
	return;
}
