...
CCardControl::CCardControl()
{
	...
	if(!m_PngCard.IsNull())
	{
		...
		m_nXDistance = m_PngCard.GetWidth()/11;
	}
...
void CCardControl::GetCardPoint(BYTE bcCardData ,CPoint &Point)
{
	...
	for (int i = 0;i<21;i++)
	{
		if(bcCardData==pbcCardData[i])
		{
			if(i>10)
			{
				Point.x = m_CardSize.cx*(i-11);
				Point.y = m_CardSize.cy;

			}else
			{
				Point.x = m_CardSize.cx*i;
				Point.y = 0;
...
VOID CCardControl::DrawCardControl(CDC * pDC)
{
	...
	for (WORD i=0;i<m_wCardCount;i++)
	{
		...
		if(m_blhideOneCard)
		{
			if(i==0)
			{
				nXImagePos=m_CardSize.cx*10;
				nYImagePos=m_CardSize.cy*1;
	...
	//pDC->Draw3dRect(OriginPoint.x,OriginPoint.y,nXDrawPos+m_MovePoint.x+25,nYDrawPos,RGB(255,0,255),RGB(255,0,255));
	return;
}
