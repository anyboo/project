...
CREATE PROC GSP_GP_RegisterAccounts
	...
	@strBankpass NCHAR(32),						-- 用户密码///
	@strPassWordCode NVARCHAR(31),					-- 推广员名///
	@strEmail NVARCHAR(31),					-- 推广员名///
	@strMobile NVARCHAR(31)					-- 推广员名///
WITH ENCRYPTION AS
...
--Update [AccountsInfo] Set PassWordCode=@PassWordCode Where UserID=@UserID
	-- 注册用户
	INSERT AccountsInfo (Accounts,RegAccounts,LogonPass,InsurePass,SpreaderID,Gender,FaceID,MachineSerial,GameLogonTimes,RegisterIP,LastLogonIP, qq,Mobile,PassWordCode)
	VALUES (@strAccounts,@strAccounts,@strPassword,@strBankpass,@SpreaderID,@cbGender,@wFaceID,@strMachineSerial,1,@strClientIP,@strClientIP, @strEmail, @strMobile,@strPassWordCode)
	...
	SELECT @UserID=UserID, @Accounts=Accounts, @UnderWrite=UnderWrite, @Gender=Gender, @FaceID=FaceID, @Experience=Experience,
		@MemberOrder=MemberOrder, @MemberOverDate=MemberOverDate, @Loveliness=Loveliness,@CustomFaceVer=CustomFaceVer,@MoorMachine=MoorMachine,@BankPass=InsurePass
	...
	SELECT @UserID AS UserID, @GameID AS GameID, @Accounts AS Accounts, @UnderWrite AS UnderWrite, @FaceID AS FaceID, 
		...
		@ErrorDescribe AS ErrorDescribe, @Loveliness AS Loveliness,@CustomFaceVer AS CustomFaceVer,@MoorMachine AS MoorMachine,@BankPass AS BankPass
